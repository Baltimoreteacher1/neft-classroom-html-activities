(() => {
  ...
  let gsTarget = "form"; // "form" | "doc" | "slides"
  let gsDocMode = "student"; // "student" | "key" (Doc only)
  ...
  // Safely turn one question into a self-contained JS object literal so the
  // generated .gs carries its own data (no dependence on this site at runtime).
  function gsQuestionLiteral(q) {
    const parts = [];
    parts.push("type: " + gsStr(q.type));
    parts.push("prompt: " + gsStr(q.prompt));
    if (q.type === "multiple-choice") {
      const choices = (q.choices || []).map((c) => gsStr(c)).join(", ");
      parts.push("choices: [" + choices + "]");
      const ci = Number.isInteger(q.correctIndex) ? q.correctIndex : 0;
      parts.push("correctIndex: " + ci);
    } else if (q.type === "short-answer") {
      parts.push("answer: " + gsStr(q.answer || ""));
    }
    if (q.standard) parts.push("standard: " + gsStr(q.standard));
    if (q.explanation) parts.push("explanation: " + gsStr(q.explanation));
    return "  { " + parts.join(", ") + " }";
  }

  // Shared header: the embedded data array + a couple of constants. Every
  // generated script begins with this so it is fully self-contained.
  function gsDataBlock(title, kind) {
    const lines = [];
    lines.push("/**");
    ...
    lines.push("var DO_NOW_QUESTIONS = [");
    const lits = state.questions.map(gsQuestionLiteral);
    lits.forEach((l, i) => lines.push(l + (i < lits.length - 1 ? "," : "")));
    lines.push("];");
    lines.push("");
    return lines.join("\n");
  }

  // Dispatcher: builds the script for the currently-selected target.
  function buildAppsScript() {
    if (gsTarget === "doc") return buildDocScript();
    if (gsTarget === "slides") return buildSlidesScript();
    return buildFormScript();
  }

  // --- Target 1: Google Form (graded quiz) ---
  function buildFormScript() {
    const title = setTitle();
    const quiz = state.quizMode;
    let s = gsDataBlock(title, "Forms");
    const L = [];
    L.push("function buildDoNow() {");
    L.push("  var form = FormApp.create(DO_NOW_TITLE);");
    L.push("  form.setDescription(DO_NOW_TITLE);");
    if (quiz) L.push("  form.setIsQuiz(true);");
    L.push("");
    L.push("  DO_NOW_QUESTIONS.forEach(function (q) {");
    L.push("    if (q.type === 'multiple-choice') {");
    L.push("      var item = form.addMultipleChoiceItem();");
    L.push("      item.setTitle(q.prompt);");
    L.push("      item.setRequired(true);");
    if (quiz) {
      L.push("      var choices = q.choices.map(function (text, i) {");
      L.push("        return item.createChoice(text, i === q.correctIndex);");
      L.push("      });");
      L.push("      item.setChoices(choices);");
      L.push("      item.setPoints(1);");
      L.push("      if (q.explanation) {");
      L.push(
      ...
      L.push("      item.setChoiceValues(q.choices);");
    }
    L.push("    } else if (q.type === 'paragraph') {");
    L.push("      form.addParagraphTextItem().setTitle(q.prompt);");
    L.push("    } else {");
    L.push("      form.addTextItem().setTitle(q.prompt);");
    L.push("    }");
    L.push("  });");
    L.push("");
    L.push("  Logger.log('Edit your form: ' + form.getEditUrl());");
    L.push("  Logger.log('Share with students: ' + form.getPublishedUrl());");
    L.push("  return form.getEditUrl();");
    L.push("}");
    return s + L.join("\n");
  }

  // --- Target 2: Google Doc (Student or Answer Key worksheet) ---
  function buildDocScript() {
    const title = setTitle();
    const showAnswers = gsDocMode === "key";
    let s = gsDataBlock(title, "Docs");
    const L = [];
    L.push("var SHOW_ANSWERS = " + (showAnswers ? "true" : "false") + ";");
    L.push("var LETTERS = 'ABCDEFGHIJ';");
    L.push("");
    L.push("function buildDoNow() {");
    L.push("  var doc = DocumentApp.create(");
    L.push("    DO_NOW_TITLE + (SHOW_ANSWERS ? ' (Answer Key)' : '')");
    L.push("  );");
    L.push("  var body = doc.getBody();");
    L.push("");
    L.push(
      "  body.appendParagraph(DO_NOW_TITLE).setHeading(DocumentApp.ParagraphHeading.HEADING1);",
    );
    ...
    L.push("    var num = (idx + 1) + '. ';");
    L.push("    var std = q.standard ? '  [' + q.standard + ']' : '';");
    L.push("    body.appendParagraph(num + q.prompt + std);");
    L.push("    if (q.type === 'multiple-choice') {");
    ...
    L.push("  });");
    L.push("");
    L.push("  doc.saveAndClose();");
    L.push("  Logger.log('Open your doc: ' + doc.getUrl());");
    L.push("  return doc.getUrl();");
    L.push("}");
    return s + L.join("\n");
  }

  // --- Target 3: Google Slides (one slide per question + answer slides) ---
  function buildSlidesScript() {
    const title = setTitle();
    let s = gsDataBlock(title, "Slides");
    const L = [];
    L.push("var LETTERS = 'ABCDEFGHIJ';");
    L.push("");
    L.push("function buildDoNow() {");
    L.push("  var deck = SlidesApp.create(DO_NOW_TITLE);");
    L.push("");
    L.push("  // Title slide");
    L.push("  var title = deck.getSlides()[0];");
    L.push("  title.getPlaceholders().forEach(function (ph) {");
    L.push("    var type = ph.getPlaceholderType();");
    L.push(
      "    if (type === SlidesApp.PlaceholderType.CENTERED_TITLE || type === SlidesApp.PlaceholderType.TITLE) {",
    );
    L.push("      ph.asShape().getText().setText(DO_NOW_TITLE);");
    L.push("    }");
    L.push("  });");
    L.push("");
    L.push("  DO_NOW_QUESTIONS.forEach(function (q, idx) {");
    ...
    L.push("    } else {");
    L.push("      atext = '(paragraph — teacher-scored)';");
    L.push("    }");
    L.push("    setPlaceholder(ans, SlidesApp.PlaceholderType.BODY, atext);");
    L.push("  });");
    L.push("");
    L.push("  Logger.log('Open your slides: ' + deck.getUrl());");
    L.push("  return deck.getUrl();");
    L.push("}");
    L.push("");
    L.push("// Helper: write text into the first matching placeholder.");
    L.push("function setPlaceholder(slide, type, text) {");
    L.push("  var ph = slide.getPlaceholder(type);");
    L.push("  if (ph) ph.asShape().getText().setText(text);");
    L.push("}");
    return s + L.join("\n");
  }

  function gsTargetLabel() {
    return gsTarget === "doc"
      ? "Google Doc"
      : gsTarget === "slides"
        ? "Google Slides"
  ...
  function setGsTarget(target) {
    gsTarget = target === "doc" || target === "slides" ? target : "form";
    $$(".gs-target-btn").forEach((b) =>
      b.setAttribute("aria-checked", String(b.dataset.gstarget === gsTarget)),
    );
    const docWrap = $("#gsDocModeWrap");
    if (docWrap) docWrap.hidden = gsTarget !== "doc";
    renderOutput();
  }
  ...
  function renderOutput() {
    ...
    out.textContent = state.questions.length
      ? buildAppsScript()
      : "Build some questions first (step 1). Then choose Form, Doc, or Slides above.";
  }
  ...
  function init() {
    ...
    $$(".gs-target-btn").forEach((b) =>
      b.addEventListener("click", () => setGsTarget(b.dataset.gstarget)),
    );
    $("#gsDocMode")?.addEventListener("change", (e) => {
      gsDocMode = e.target.value === "key" ? "key" : "student";
      renderOutput();
    });
