...
const analysisCatalog = [
  ["descriptive","Descriptive Snapshot","Mean, median, range, proficiency rate, and quick class health check.","📌"],
  ["distribution","Distribution + Spread","Histogram, spread bands, and whether scores are clustered or widely varied.","📊"],
  ["standardPriority","Priority Standards","Ranks standards by reteach need and percent below target.","🎯"],
  ["growth","Growth Trajectory","Shows first-to-latest progress by class and student.","📈"],
  ["prediction","Prediction Analysis","Projects next likely class and student performance from trend data.","🔮"],
  ["correlation","Correlation Studio","Connects scores to another numeric measure, such as attendance.","🔗"],
  ["subgroup","Subgroup Comparison","Compares outcomes by class, category, or supplemental group.","🧭"],
  ["outliers","Outlier Scan","Finds unusually high or low patterns worth checking.","🚩"],
  ["consistency","Consistency Check","Finds students with uneven performance across skills.","🧩"],
  ["intervention","Group Builder","Creates practical reteach, guided, monitor, and extension groups.","👥"],
  ["equity","Access Gap Check","Highlights possible access/support gaps between groups.","⚖️"],
  ["reliability","Signal Strength","Checks whether the data is strong enough to trust for decisions.","✅"]
];
...
const state = {
  ...
  advanced: {
    selected: ["descriptive","distribution","standardPriority","growth","prediction","intervention"],
    corrField: "",
...
function mean(arr){ const vals = arr.filter(Number.isFinite); return vals.length ? vals.reduce((a,b)=>a+b,0)/vals.length : NaN; }
function median(arr){
...
function sd(arr){
  ...
  const m = mean(vals);
  return Math.sqrt(vals.reduce((acc,v)=>acc+Math.pow(v-m,2),0)/(vals.length-1));
}
function round(n, digits=1){ return Number.isFinite(n) ? Number(n.toFixed(digits)) : "—"; }
function pct(n, digits=0){ return Number.isFinite(n) ? `${round(n,digits)}%` : "—"; }
function unique(arr){ return [...new Set(arr.filter(v => v !== null && v !== undefined && String(v).trim() !== "").map(v => String(v).trim()))]; }
function clamp(n,min,max){ return Math.max(min, Math.min(max,n)); }
function scoreBand(score,target=70){
  if(!Number.isFinite(score)) return {label:"Missing", cls:"", color:"#f1f5f9"};
  if(score < Math.max(50,target-20)) return {label:"Urgent", cls:"red", color:"#fecaca"};
  if(score < target) return {label:"Reteach", cls:"gold", color:"#fde68a"};
  if(score < target+15) return {label:"On Track", cls:"blue", color:"#bfdbfe"};
...
function inferFields(headers){
  return {
    student: inferField(headers, ["Student Name","Student","Name","StudentName","Full Name","Scholar"]),
    score: inferField(headers, ["Score (%)","Score","Percent","Percentage","Grade","Result","Points Earned","Performance"]),
    standard: inferField(headers, ["Standard","Skill","Objective","Domain","Learning Target","Item Standard"]),
    assessment: inferField(headers, ["Assessment","Test","Quiz","Assignment","Window","Unit","Date","Benchmark"]),
    className: inferField(headers, ["Class/Period","Class","Period","Section","Course","Teacher"]),
    date: inferField(headers, ["Date","Assessment Date","Test Date","Admin Date"])
  };
}
...
function normalizeRows(rows){
  state.rawRows = rows.filter(r => r && Object.values(r).some(v => String(v ?? "").trim() !== ""));
...
function rebuildRecords(){
  ...
  state.records = state.rawRows.map((r, idx) => {
    const score = toNumber(r[f.score]);
    const student = String(r[f.student] ?? `Student ${idx+1}`).trim() || `Student ${idx+1}`;
    const standard = String(r[f.standard] ?? "General").trim() || "General";
    const assessment = String(r[f.assessment] ?? "Assessment").trim() || "Assessment";
...
function resetFiltersToAll(){
  ...
  if(!students.includes(state.filters.focusStudent)) state.filters.focusStudent = "All";
}
function getAvailable(){
  return {
    students: unique(state.records.map(d => d.student)).sort((a,b)=>a.localeCompare(b)),
    standards: unique(state.records.map(d => d.standard)).sort((a,b)=>a.localeCompare(b)),
    classes: unique(state.records.map(d => d.className)).sort((a,b)=>a.localeCompare(b)),
    assessments: sortAssessments(unique(state.records.map(d => d.assessment)))
  };
}
...
function getFilteredRecords(){
  const targetStudents = state.filters.focusStudent === "All" ? state.filters.selectedStudents : [state.filters.focusStudent];
  return state.records.filter(d => {
    return (state.filters.className === "All" || d.className === state.filters.className) &&
           (state.filters.assessment === "All" || d.assessment === state.filters.assessment) &&
...
function studentSummaries(data=getFilteredRecords()){
  ...
  return [...byStudent.entries()].map(([student, rows]) => {
    const scores = rows.map(r=>r.score).filter(Number.isFinite);
    const assessments = sortAssessments(unique(rows.map(r=>r.assessment)));
    const ordered = assessments.map(a => mean(rows.filter(r=>r.assessment === a).map(r=>r.score)));
    ...
    return {
      ...
      first,
      latest,
      growth: Number.isFinite(first) && Number.isFinite(latest) ? latest-first : NaN,
      rows
    };
...
function standardSummaries(data=getFilteredRecords()){
  ...
  return [...byStandard.entries()].map(([standard, rows]) => {
    const scores = rows.map(r=>r.score).filter(Number.isFinite);
    return {
      standard,
      n: scores.length,
      ...
      belowPct: scores.length ? scores.filter(s=>s<target).length/scores.length*100 : NaN,
      min: Math.min(...scores),
      max: Math.max(...scores)
    };
...
function assessmentSummaries(data=getFilteredRecords()){
  ...
  return sortAssessments([...byAssessment.keys()]).map((assessment, i) => {
    ...
    const scores = rows.map(r=>r.score).filter(Number.isFinite);
    return { assessment, index:i+1, n:scores.length, avg:mean(scores), median:median(scores), sd:sd(scores), target:Number(state.filters.target) };
  });
}
function linearRegression(points){
  ...
  const slope = pts.reduce((acc,p)=>acc+(p.x-xbar)*(p.y-ybar),0)/denom;
  const intercept = ybar - slope*xbar;
  const ssTot = pts.reduce((acc,p)=>acc+Math.pow(p.y-ybar,2),0);
  const ssRes = pts.reduce((acc,p)=>acc+Math.pow(p.y-(intercept+slope*p.x),2),0);
  const r2 = ssTot === 0 ? 1 : 1 - ssRes/ssTot;
  return { slope, intercept, r2, predict:x => intercept + slope*x, n:pts.length };
}
function pearson(xs, ys){
  ...
  const denX = Math.sqrt(pairs.reduce((acc,p)=>acc+Math.pow(p.x-xbar,2),0));
  const denY = Math.sqrt(pairs.reduce((acc,p)=>acc+Math.pow(p.y-ybar,2),0));
  return { r: denX && denY ? num/(denX*denY) : NaN, n:pairs.length, pairs };
}
function describeCorrelation(r){
  ...
  const strength = abs >= .75 ? "very strong" : abs >= .55 ? "strong" : abs >= .35 ? "moderate" : abs >= .2 ? "weak" : "very weak";
...
function openExplain(key){
  const entry = HELP[key] || {title:"Explanation", body:"This feature translates the data into teacher-friendly language."};
  document.getElementById("modalTitle").textContent = entry.title;
  document.getElementById("modalBody").innerHTML = `<p>${escapeHtml(entry.body)}</p><div class="translation"><strong>Best use:</strong> Treat this as a planning signal. Pair it with what you know from classwork, student talk, attendance, language demands, and recent instruction before making a final decision.</div>`;
...
function rebuildRecordsNoRender(){
  ...
  state.records = state.rawRows.map((r, idx) => ({
    id: idx,
    student: String(r[f.student] ?? `Student ${idx+1}`).trim() || `Student ${idx+1}`,
    score: toNumber(r[f.score]),
    standard: String(r[f.standard] ?? "General").trim() || "General",
    assessment: String(r[f.assessment] ?? "Assessment").trim() || "Assessment",
...
function clearAll(){
  ...
  Object.assign(state, {
    ...
    extraHeaders:[],
    records:[],
    filters:{ className:"All", assessment:"All", focusStudent:"All", selectedStudents:[], selectedStandards:[], target:70 },
    fields:{ student:"", score:"", standard:"", assessment:"", className:"", date:"" },
    advanced:{ selected:["descriptive","distribution","standardPriority","growth","prediction","intervention"], corrField:"", subgroupField:"className", projectionSteps:1, minGroupSize:2 },
    planning:{ purpose:"reteach", minutes:45, cycle:"Tomorrow", audience:"teacher", includeESOL:true, includeFamily:true },
    charts:{},
    analysisRun:false
  });
  render();
  ...
  reader.onload = e => {
    try{
      ...
      if(name.endsWith(".csv")){
        ...
        if(typeof XLSX === "undefined") throw new Error("Spreadsheet library did not load. Try CSV or check internet access.");
      ...
      alert("I could not read that file. Try saving it as .xlsx or .csv.\n\n" + err.message);
    }
  };
...
function appendSupplement(rows){
  ...
  if(candidates.length && !candidates.includes(state.advanced.corrField)) state.advanced.corrField = candidates[0];
...
function numericSupplementFields(){
  ...
  return state.extraHeaders.filter(h => {
    const vals = state.extraRows.map(r => toNumber(r[h])).filter(Number.isFinite);
    return vals.length >= Math.max(3, Math.ceil(state.extraRows.length*.35));
  }).filter(h => {
...
function categoricalSupplementFields(){
  ...
  return state.extraHeaders.filter(h => {
    ...
    const numeric = state.extraRows.map(r => toNumber(r[h])).filter(Number.isFinite).length;
    return vals.length >= 2 && vals.length <= Math.max(12, state.extraRows.length*.45) && numeric < state.extraRows.length*.7;
  });
}
function detectSupplementStudentField(){
  return inferField(state.extraHeaders, ["Student Name","Student","Name","StudentName","Full Name","Scholar"]) || state.extraHeaders[0] || "";
}
...
function toggleFilter(kind, value){
  const arr = kind === "students" ? state.filters.selectedStudents : state.filters.selectedStandards;
...
function setAll(kind, checked){
  ...
  if(kind === "students") state.filters.selectedStudents = checked ? [...avail.students] : [];
  if(kind === "standards") state.filters.selectedStandards = checked ? [...avail.standards] : [];
  state.analysisRun = false;
  persistLite();
  render();
}
...
function runSelectedAnalyses(){
  ...
  persistLite();
  render();
  setTimeout(()=>document.getElementById("analysisResults")?.scrollIntoView({behavior:"smooth", block:"start"}), 80);
}
function selectRecommendedAnalyses(){
  state.advanced.selected = ["descriptive","distribution","standardPriority","growth","prediction","intervention","reliability"];
  state.analysisRun = false;
  render();
}
...
function chart(id, config){
  ...
  state.charts[id] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins:{ legend:{ position:"bottom", labels:{ usePointStyle:true, boxWidth:8 } }, tooltip:{ mode:"nearest", intersect:false }, ...(config.options?.plugins || {}) },
      scales: config.type === "pie" || config.type === "doughnut" || config.type === "radar" ? config.options?.scales : {
        y:{ beginAtZero:true, grid:{ color:"#eef3f8" }, ticks:{ color:"#64748b" } },
...
function initCharts(){
  ...
  if(state.tab === "overview"){
    ...
    chart("overviewHistogram", { type:"bar", data:{ labels:bins.map(b=>b.label), datasets:[{ label:"Records", data:bins.map(b=>scores.filter(s=>s>=b.min && s<=b.max).length), backgroundColor:"#0f766e", borderRadius:10 }] } });
    const ss = standardSummaries(data).slice(0,8).reverse();
    chart("overviewStandards", { type:"bar", data:{ labels:ss.map(s=>s.standard), datasets:[{ label:"Average score", data:ss.map(s=>round(s.avg,1)), backgroundColor:"#2563eb", borderRadius:10 }] }, options:{ indexAxis:"y", scales:{ x:{ suggestedMax:100 } } } });
  }
  if(state.tab === "trends"){
    const assess = assessmentSummaries(data);
    chart("trendLine", { type:"line", data:{ labels:assess.map(a=>a.assessment), datasets:[{ label:"Class average", data:assess.map(a=>round(a.avg,1)), borderColor:"#0f766e", backgroundColor:"rgba(15,118,110,.12)", fill:true, tension:.35, pointRadius:5 }, { label:"Target", data:assess.map(a=>Number(state.filters.target)), borderColor:"#b7791f", borderDash:[6,6], pointRadius:0 }] }, options:{ scales:{ y:{ suggestedMin:0, suggestedMax:100 } } } });
  }
...
function renderAnalysisCharts(){
  ...
  if(selected.includes("distribution")){
    ...
    chart("analysisDistributionChart", { type:"bar", data:{ labels:bins.map(b=>b.label), datasets:[{ label:"Number of score records", data:bins.map(b=>scores.filter(s=>s>=b.min && s<=b.max).length), backgroundColor:"#4f46e5", borderRadius:10 }] } });
  }
  if(selected.includes("standardPriority")){
    const ss = standardSummaries(data).slice(0,10).reverse();
    chart("analysisStandardChart", { type:"bar", data:{ labels:ss.map(s=>s.standard), datasets:[{ label:"% below target", data:ss.map(s=>round(s.belowPct,0)), backgroundColor:"#c2410c", borderRadius:10 }] }, options:{ indexAxis:"y", scales:{ x:{ suggestedMax:100 } } } });
  }
  if(selected.includes("growth")){
    const summaries = studentSummaries(data).filter(s=>Number.isFinite(s.growth)).sort((a,b)=>a.growth-b.growth).slice(0,12);
    chart("analysisGrowthChart", { type:"bar", data:{ labels:summaries.map(s=>s.student), datasets:[{ label:"Growth points", data:summaries.map(s=>round(s.growth,1)), backgroundColor:"#0f766e", borderRadius:10 }] } });
  }
  if(selected.includes("prediction")){
    const pred = predictionAnalysis(data);
    chart("analysisPredictionChart", { type:"line", data:{ labels:pred.labels, datasets:[{ label:"Actual average", data:pred.actual, borderColor:"#2563eb", backgroundColor:"rgba(37,99,235,.10)", fill:false, tension:.35, pointRadius:5 }, { label:"Projection", data:pred.projectedSeries, borderColor:"#b7791f", borderDash:[6,6], tension:.35, pointRadius:5 }] }, options:{ scales:{ y:{ suggestedMin:0, suggestedMax:100 } } } });
  }
  if(selected.includes("correlation")){
    const corr = correlationAnalysis(data);
    if(corr && corr.pairs.length){
      chart("analysisCorrelationChart", { type:"scatter", data:{ datasets:[{ label:`${corr.field} vs Score`, data:corr.pairs.map(p=>({x:p.x,y:p.y})), backgroundColor:"#0f766e", pointRadius:5 }] }, options:{ scales:{ x:{ title:{display:true, text:corr.field} }, y:{ title:{display:true,text:"Score"}, suggestedMin:0, suggestedMax:100 } } } });
    }
  }
  if(selected.includes("subgroup") || selected.includes("equity")){
    const sub = subgroupAnalysis(data);
    if(sub.groups.length){
      chart("analysisSubgroupChart", { type:"bar", data:{ labels:sub.groups.map(g=>g.label), datasets:[{ label:"Average score", data:sub.groups.map(g=>round(g.avg,1)), backgroundColor:"#7c3aed", borderRadius:10 }] }, options:{ scales:{ y:{ suggestedMax:100 } } } });
    }
  }
}
    ...
    <header class="topbar no-print">
      <div class="brand">
        ...
        <div class="brand-copy">
          <span class="brand-title">Teacher Data Dashboard</span>
...
function renderOverview(){
  ...
  const scores = data.map(d=>d.score);
  const target = Number(state.filters.target);
  const prof = scores.length ? scores.filter(s=>s>=target).length/scores.length*100 : NaN;
...
function overviewTranslation(scores, prof, standards){
  ...
  const avg = mean(scores), spread = Math.max(...scores)-Math.min(...scores), target = Number(state.filters.target);
  const status = avg >= target ? "The class average is currently above the target" : "The class average is currently below the target";
  const spreadText = spread >= 35 ? "The wide score spread suggests you should plan differentiated groups rather than one whole-class activity." : "The score spread is manageable, so a short whole-class reteach plus targeted checks may work.";
  const low = standards[0] ? `The clearest standard to prioritize is ${escapeHtml(standards[0].standard)}.` : "No single standard is clearly isolated yet.";
  ...
  return `
    <div class="container">
      ...
      <section class="card">
        ...
        ${table(["Assessment","Records","Average","Median","Spread"], assess.map(a=>[a.assessment,a.n,pct(a.avg),pct(a.median),round(a.sd,1)]))}
      </section>
    </div>
  `;
}
function renderPatterns(){
  const data = getFilteredRecords();
  const students = state.filters.focusStudent === "All" ? state.filters.selectedStudents : [state.filters.focusStudent];
...
function renderGroups(){
  ...
  const cards = [
    ["urgent","red","Intensive Reteach","Students need immediate small-group support with modeling, vocabulary/visual support, and quick checks."],
    ["strategic","gold","Guided Practice","Students are near target but need structured practice and feedback."],
    ["monitor","blue","On Track / Monitor","Students are meeting the target; keep checking for retention and precision."],
    ["extension","green","Extension","Students are ready for challenge tasks, explanation prompts, or peer-coaching roles."]
  ];
  return `
    <div class="container">
      ${renderFilters()}
      ...
      <section class="card">
        ...
        ${table(["Student","Average","Growth","Current Band","Suggested Teacher Move"], summaries.map(s=>[s.student,pct(s.avg),Number.isFinite(s.growth)?`${round(s.growth,1)} pts`:"—",scoreBand(s.avg,target).label,suggestMove(s,target)]))}
      </section>
    </div>
  `;
}
...
function suggestMove(s,target){
  if(s.avg < target-15) return "Reteach prerequisite skill with visual model, sentence frame, and immediate guided check.";
  if(s.avg < target) return "Use guided practice with feedback; check one misconception before independent work.";
  if(s.avg < target+15) return "Continue grade-level practice; ask for written explanation to confirm understanding.";
    ...
    <div class="container">
      ...
      <section class="card soft">
        ...
        <div class="analysis-picker">
          ${analysisCatalog.map(([key,title,desc,icon]) => `
...
function renderAdvancedSettings(){
  ...
  if(corrFields.length && !state.advanced.corrField) state.advanced.corrField = corrFields[0];
  return `
...
function renderAnalysisResults(){
  const data = getFilteredRecords();
  const selected = state.advanced.selected;
  if(!selected.length) return `<div class="empty"><h2>No analyses selected.</h2><p>Select at least one analysis option above.</p></div>`;
...
function resultStandardPriority(data){
  ...
  return `
    ...
    ${table(["Priority","Standard","Average","% Below Target","Records"], standards.slice(0,8).map((s,i)=>[i+1,s.standard,pct(s.avg),pct(s.belowPct),s.n]))}
  `;
}
function resultGrowth(data){
  const summaries = studentSummaries(data).filter(s=>Number.isFinite(s.growth)).sort((a,b)=>a.growth-b.growth);
  ...
  return `
    ...
    ${table(["Student","First","Latest","Growth","Planning Note"], summaries.slice(0,10).map(s=>[s.student,pct(s.first),pct(s.latest),Number.isFinite(s.growth)?`${round(s.growth,1)} pts`:"—",s.growth<0?"Check misconception or access barrier":s.growth<5?"Monitor and give targeted feedback":"Positive movement"]))}
  `;
}
function predictionAnalysis(data){
  const assess = assessmentSummaries(data);
  const points = assess.map((a,i)=>({x:i+1,y:a.avg,label:a.assessment}));
  ...
  let projected = NaN, confidence = "Low";
  if(reg){
    projected = clamp(reg.predict(points.length+steps),0,100);
    labels.push(`Projected +${steps}`);
    actual.push(null);
    projectedSeries[projectedSeries.length-1] = round(assess[assess.length-1]?.avg,1);
    projectedSeries.push(round(projected,1));
    confidence = reg.n >= 4 && reg.r2 >= .55 ? "Moderate" : reg.n >= 3 ? "Cautious" : "Low";
  }
  return { assess, points, reg, labels, actual, projectedSeries, projected, confidence, steps };
}
function resultPrediction(data){
  const pred = predictionAnalysis(data);
  const target = Number(state.filters.target);
  if(!pred.reg) return `<div class="empty"><h3>Prediction needs at least two assessment windows.</h3><p>Upload or select data with multiple assessments to project a trend.</p></div>`;
  const direction = pred.reg.slope > 1 ? "upward" : pred.reg.slope < -1 ? "downward" : "mostly flat";
  const risk = pred.projected >= target ? "on pace to meet the selected target" : "not yet on pace to meet the selected target";
  return `
    ...
    ${table(["Metric","Value","Teacher Meaning"], [["Trend per assessment",`${round(pred.reg.slope,1)} pts`, pred.reg.slope>0?"Scores are increasing over time.":pred.reg.slope<0?"Scores are decreasing over time.":"Scores are mostly stable."],["Projected average",pct(pred.projected), pred.projected>=target?"Likely maintain or exceed target if trend continues.":"Plan reteach or intervention before the next assessment."],["Confidence",pred.confidence,"Use as an early-warning signal, not a final decision."]])}
  `;
}
function resultCorrelation(data){
  const corr = correlationAnalysis(data);
  if(!corr) return `<div class="empty"><h3>Correlation needs supplemental numeric data.</h3><p>Open Data Manager and append a file with Student Name plus a numeric field such as Attendance %, Missing Assignments, Behavior Points, or Intervention Minutes.</p></div>`;
  return `
    <div class="chart-wrap"><canvas id="analysisCorrelationChart"></canvas></div>
    <div class="translation" style="margin-top:14px"><strong>What this means:</strong> ${escapeHtml(corr.field)} and score show a <strong>${describeCorrelation(corr.r)}</strong> relationship (r = ${round(corr.r,2)}, n = ${corr.n}). ${Math.abs(corr.r)>=.35 ? "This pattern is strong enough to discuss as a planning clue." : "This pattern is weak, so avoid over-interpreting it."}</div>
  `;
}
function correlationAnalysis(data){
  ...
  byStudent.forEach(s => {
    ...
    if(Number.isFinite(x) && Number.isFinite(y)){
      xs.push(x); ys.push(y); pairs.push({student:s.student,x,y});
    }
  });
...
function subgroupAnalysis(data){
  ...
  byStudent.forEach(s => {
    ...
    else if(field === "assessment") label = s.rows[0]?.assessment || "Assessment";
    else if(field === "standard") label = "By standard";
    else if(field.startsWith("extra:")){
      const f = field.slice(6);
      label = sup.get(s.student)?.[f] || "";
    }
    if(label) groupRows.push({ label:String(label), avg:s.avg, student:s.student });
  });
  const byGroup = grouped(groupRows, r=>r.label);
  const groups = [...byGroup.entries()].map(([label, rows]) => ({ label, n:rows.length, avg:mean(rows.map(r=>r.avg)), min:Math.min(...rows.map(r=>r.avg)), max:Math.max(...rows.map(r=>r.avg)) })).sort((a,b)=>a.avg-b.avg);
  const gap = groups.length >= 2 ? groups[groups.length-1].avg - groups[0].avg : NaN;
  return { field, groups, gap };
}
function resultSubgroup(data){
  const sub = subgroupAnalysis(data);
  if(!sub.groups.length) return `<div class="empty"><h3>No subgroup field available.</h3><p>Try class period or append supplemental category data.</p></div>`;
  return `
    ...
    ${table(["Group","Students","Average","Lowest","Highest"], sub.groups.map(g=>[g.label,g.n,pct(g.avg),pct(g.min),pct(g.max)]))}
  `;
}
function resultEquity(data){
  const sub = subgroupAnalysis(data);
  if(sub.groups.length < 2) return `<div class="empty"><h3>Equity check needs at least two groups.</h3><p>Select class period, assessment group, or upload a supplemental category field.</p></div>`;
  const low = sub.groups[0], high = sub.groups[sub.groups.length-1];
  return `
...
function resultOutliers(data){
  ...
  const m = mean(avgs), spread = sd(avgs);
  const rows = summaries.map(s => ({...s, z: spread ? (s.avg-m)/spread : 0 })).filter(s=>Math.abs(s.z)>=1.2).sort((a,b)=>a.avg-b.avg);
  return `
    ...
    ${rows.length ? table(["Student","Average","Distance from Group","Possible Use"], rows.map(s=>[s.student,pct(s.avg),`${round(s.z,2)} z`,s.z<0?"Urgent check-in / misconception review":"Extension or peer explanation role"])) : ""}
  `;
}
function resultConsistency(data){
  ...
  return `
    ...
    ${table(["Student","Average","Variation","Range","Planning Meaning"], top.map(s=>[s.student,pct(s.avg),round(s.sd,1),round(s.spread,0),s.sd>=15?"Uneven skill profile — diagnose by standard":"Fairly consistent pattern"]))}
  `;
}
...
function resultReliability(data){
  ...
  const checks = [
    ["Student sample", students >= 8, `${students} students selected`, students >= 8 ? "Stronger for group-level planning." : "Use caution; small groups can swing easily."],
    ["Assessment windows", assessments >= 2, `${assessments} assessment(s)`, assessments >= 2 ? "Trend/prediction can be attempted." : "Prediction needs more than one assessment."],
    ["Standards represented", standards >= 2, `${standards} standard(s)`, standards >= 2 ? "Standard comparison is useful." : "Standard comparison is limited."],
    ["Readable score rows", missingScoreRows === 0, `${missingScoreRows} row(s) skipped`, missingScoreRows === 0 ? "All uploaded rows were usable." : "Some rows lacked usable student/score data."]
  ];
  return `
...
function renderCommandCenter(data){
  ...
  const standards = standardSummaries(data);
  const pred = predictionAnalysis(data);
  const groups = buildGroups(studentSummaries(data), Number(state.filters.target));
  ...
  const predictionText = pred.reg ? `${pct(pred.projected)} projected next average` : "Add another assessment window for prediction";
  const confidence = pred.reg ? `${pred.confidence} confidence` : "Needs trend data";
  return `
...
function updatePlanning(key, value){
  if(key === "minutes") state.planning[key] = clamp(Number(value)||45, 15, 120);
  else if(key === "includeESOL" || key === "includeFamily") state.planning[key] = Boolean(value);
...
function priorityReason(p, i){
  ...
  if(i === 0) return `Highest urgency: ${pct(p.belowPct)} below target and average ${pct(p.avg)}.`;
  if(p.belowPct >= 50) return `Many students are below target (${pct(p.belowPct)}), so this may need a second support cycle.`;
  if(p.sd >= 15) return "Scores are uneven, suggesting targeted groups rather than whole-class reteach only.";
  return "Monitor with a short check after the highest-priority standard is addressed.";
}
function buildPlanningGroups(summaries=studentSummaries(), priorities=standardSummaries(), target=Number(state.filters.target)){
  ...
  const mk = (name, kind, students, focusText, teacherMove, task, check, support) => ({
    name, kind, students, avg: mean(students.map(s=>s.avg)), focus: focusText, teacherMove, task, check, support
  });
  if(purpose === "mixed"){
    const buckets = [[],[],[],[]];
    sortedHigh.forEach((s,i)=>buckets[i%4].push(s));
    return buckets.filter(g=>g.length).map((g,i)=>mk(
      `Mixed Team ${i+1}`,"blue",g,`Collaborative practice on ${focus}`,
  ...
  if(purpose === "extension"){
    return [
      mk("Precision Reteach","red",[...base.urgent,...base.strategic],focus,"Model the misconception, then have students repair an incorrect solution.","Guided practice with immediate feedback and one explain-your-thinking item.","One targeted exit-ticket item on the priority standard.","Provide visuals, word bank, and a first-step sentence frame."),
      mk("Grade-Level Application","blue",base.monitor,focus,"Give grade-level practice with a required explanation, not just an answer.","Solve independently, compare methods, and revise explanations.","Collect one written explanation for accuracy and vocabulary.","Offer optional sentence kernels for explanations."),
      mk("Extension Studio","green",base.extension,`Extend ${focus}`,"Use challenge, error analysis, or student-created examples.","Create a harder version of the problem and an answer key.","Students defend why their problem matches the standard.","Push academic vocabulary and precision."
    )].filter(g=>g.students.length);
  }
  if(purpose === "intervention"){
    const priority = sortedLow.filter(s=>s.avg < target || s.growth < 0);
    const watch = sortedLow.filter(s=>s.avg >= target && s.avg < target+10);
    const ready = sortedLow.filter(s=>s.avg >= target+10);
    return [
      mk("Intervention First","red",priority,focus,"Start with prerequisite language and one modeled example; do not begin with independent practice.","Teacher table: I do → We do → You do one item with feedback.","Ask students to identify the first step and explain why.","Use visual model, vocabulary preview, reduced language load, and frequent checks."),
      mk("Watch List","gold",watch,focus,"Use guided practice and targeted feedback before students work alone.","Two scaffolded problems with a partner check.","One individual check before release.","Keep sentence frames available but fade them as students gain confidence."),
      mk("Independent / Extension","green",ready,focus,"Keep students on grade-level work with an extension option.","Independent practice plus error analysis.","One written explanation or peer teach-back.","Require precise math vocabulary."
    )].filter(g=>g.students.length);
  }
  if(purpose === "peer"){
    const lows = [...base.urgent,...base.strategic];
    const highs = [...base.extension,...base.monitor].sort((a,b)=>b.avg-a.avg);
    const pairs = lows.map((s,i)=>[s, highs[i % Math.max(1, highs.length)]].filter(Boolean));
    return pairs.map((pair,i)=>mk(
      `Peer Support Pair ${i+1}`,"teal",pair,focus,
  ...
  if(purpose === "testprep"){
    return [
      mk("Must-Fix Standard","red",base.urgent,focus,"Short reteach on the highest-risk tested skill.","One released-style problem with annotation and answer-choice elimination.","Students explain why two wrong answers are wrong.","Preteach vocabulary and underline command words."),
      mk("Almost There","gold",base.strategic,focus,"Focus on accuracy, units, and avoiding common traps.","Two mixed-review problems with immediate feedback.","Error analysis: find and fix one mistake.","Use a checklist for steps and labels."),
      mk("On Track Review","blue",base.monitor,focus,"Maintain fluency and written reasoning.","Timed-but-calm practice set with explanation item.","Students score their confidence and revise one answer.","Offer optional vocabulary support."),
      mk("Challenge Review","green",base.extension,`Advanced application of ${focus}`,"Use multi-step or constructed-response problems.","Create a test-prep hint card for classmates.","Peer review for precision.","Require formal academic vocabulary."
    )].filter(g=>g.students.length);
  }
  return [
    mk("Intensive Reteach","red",base.urgent,focus,"Use direct modeling with visuals, vocabulary, and immediate teacher feedback.","One worked example, one guided problem, one independent check.","Ask: ‘What is the first step, and how do you know?’","Add a visual model, vocabulary bank, sentence frame, and chunked steps."),
    mk("Guided Practice","gold",base.strategic,focus,"Use scaffolded practice and catch misconceptions before independent work.","Two guided items with partner talk and teacher feedback.","Students complete one similar item independently.","Use sentence starters and a worked-example reference."),
    mk("Monitor / Grade-Level","blue",base.monitor,focus,"Keep students on grade-level tasks while checking precision.","Independent practice with a required written explanation.","Collect one explanation and one computation check.","Offer vocabulary supports as optional tools."),
    mk("Extension","green",base.extension,`Extend ${focus}`,"Push reasoning, error analysis, and student-created examples.","Create a challenge problem, solve it, and explain the strategy.","Students defend why their answer is reasonable.","Require precise academic language and multiple representations.")
  ].filter(g=>g.students.length);
}
function buildAgenda(priorities=standardSummaries(), groups=buildPlanningGroups(), minutes=Number(state.planning.minutes)||45){
  const focus = priorities[0]?.standard || "priority skill";
  const m = clamp(minutes,15,120);
  const intro = Math.max(4, Math.round(m*.12));
  const model = Math.max(7, Math.round(m*.22));
  const groupsTime = Math.max(12, Math.round(m*.38));
  const share = Math.max(4, Math.round(m*.12));
  const exit = Math.max(5, m-intro-model-groupsTime-share);
  return [
    [`${intro} min`,"Launch + goal",`Name the focus: ${focus}. Students preview vocabulary and explain what the task is asking.`],
    [`${model} min`,"Model the high-leverage move",`Teacher models one example and pauses for students to identify the first step, representation, or formula.`],
    [`${groupsTime} min`,"Flexible group work",`${groups.length} group(s) work on differentiated tasks while the teacher starts with the highest-need group.`],
    [`${share} min`,"Compare strategies",`Students share one strategy, misconception fix, or sentence using academic vocabulary.`],
    [`${exit} min`,"Exit evidence",`Students complete a short check that can be uploaded back into Teacher Data Dashboard.`]
  ];
}
function buildAdminBriefText(){
  ...
  const groups = buildPlanningGroups(studentSummaries(data), priorities, target);
  const pred = predictionAnalysis(data);
  return [
    "Teacher Data Dashboard — Admin/Coach Brief",
    ...
    priorities[0] ? `Rationale: ${pct(priorities[0].belowPct)} below target; average ${pct(priorities[0].avg)}.` : "Rationale: add standard data for a clearer priority.",
    pred.reg ? `Prediction: ${pct(pred.projected)} projected next average (${pred.confidence} confidence).` : "Prediction: more assessment windows needed.",
    "",
    "Instructional response:",
    ...groups.map(g=>`- ${g.name}: ${g.students.length} student(s). ${g.teacherMove}`),
    "",
    "Evidence to collect next: 3-question exit ticket with one fluency item, one grade-level item, and one written explanation."
  ].join("\n");
}
function buildStudentFamilySummary(s, priority, target=Number(state.filters.target), lang="EN"){
  const band = scoreBand(s.avg,target).label;
  const growth = Number.isFinite(s.growth) ? `${round(s.growth,1)} points` : "more data needed";
  if(lang === "ES"){
...
function buildPlanningPrompt(){
  ...
  return [
    ...
    priorities[0] ? `Why this is priority: average ${pct(priorities[0].avg)}; ${pct(priorities[0].belowPct)} below target.` : "Why this is priority: standard-level data was not available.",
    pred.reg ? `Prediction signal: next projected average ${pct(pred.projected)} (${pred.confidence} confidence).` : "Prediction signal: not enough assessment windows for a trend projection.",
    "",
    "Flexible groups:",
    ...groups.map(g=>`- ${g.name}: ${listNames(g.students)}. Focus: ${g.focus}. Teacher move: ${g.teacherMove}`),
    "",
...
function renderPlanningStudio(){
  ...
  const agenda = buildAgenda(priorities, groups, Number(state.planning.minutes));
  const pred = predictionAnalysis(data);
    ...
    <div class="container">
        ...
        <div class="planning-controls no-print">
          <div class="control">
            ...
            <select onchange="updatePlanning('purpose', this.value)">
              ${[
                ["reteach","Targeted reteach"],["mixed","Mixed collaboration"],["extension","Extension + support split"],["intervention","Intervention priority"],["peer","Peer support partnerships"],["testprep","Test-prep rotation"]
              ].map(([v,l])=>`<option value="${v}" ${state.planning.purpose===v?'selected':''}>${l}</option>`).join("")}
            </select>
          </div>
          <div class="control">
          ...
          <div class="control">
            ...
            <select onchange="updatePlanning('audience', this.value)">
              ${[["teacher","Teacher"],["admin","Admin/Coach"],["family-en","Family/Student EN"],["family-es","Family/Student ES"]].map(([v,l])=>`<option value="${v}" ${state.planning.audience===v?'selected':''}>${l}</option>`).join("")}
            </select>
          </div>
        </div>
      ...
      <section class="grid grid-2">
        <div class="card">
          ...
          ${table(["Priority","Standard / Skill","Why it matters","Recommended Action"], priorities.map((p,i)=>[i+1,p.standard,priorityReason(p,i),i===0?"Whole/small-group reteach now":i===1?"Second support cycle or spiral review":"Monitor or embed in practice"]))}
        </div>
      ...
      <section class="card">
        ...
        <div class="family-grid">
          ${summaries.slice(0,10).map(s=>`<article class="family-card"><h3>${escapeHtml(s.student)}</h3><p class="hint">${escapeHtml(buildStudentFamilySummary(s, topFocus, target, familyLang))}</p></article>`).join("")}
        </div>
      </section>
    </div>
  `;
...
function buildPlanningStudioText(){
  ...
  const agenda = buildAgenda(priorities, groups, Number(state.planning.minutes));
  return [
    "Teacher Data Dashboard — Planning Studio",
    `Planning purpose: ${planningPurposeLabel()}`,
    `Cycle: ${state.planning.cycle}`,
    `Time: ${state.planning.minutes} minutes`,
    `Priority focus: ${priorities[0]?.standard || "Not available"}`,
    priorities[0] ? `Priority rationale: ${pct(priorities[0].belowPct)} below target; average ${pct(priorities[0].avg)}.` : "Priority rationale: Add standard data for stronger ranking.",
    "",
    "Next-day agenda:",
    ...agenda.map(a=>`- ${a[0]} ${a[1]}: ${a[2]}`),
    "",
    "Small-group plan:",
    ...groups.map(g=>`- ${g.name} (${g.students.length}): ${listNames(g.students)}. Focus: ${g.focus}. Move: ${g.teacherMove}. Check: ${g.check}`),
    "",
...
function copyPlanningStudioSummary(){
  ...
  navigator.clipboard?.writeText(text).then(()=>toast("Planning summary copied."),()=>alert(text));
}
function copyPlanningPrompt(){
  const text = buildPlanningPrompt();
  navigator.clipboard?.writeText(text).then(()=>toast("AI prompt copied."),()=>alert(text));
}
function copyFamilySummaries(){
  ...
  const text = studentSummaries().map(s=>buildStudentFamilySummary(s, focus, target, lang)).join("\n\n");
  navigator.clipboard?.writeText(text).then(()=>toast("Student/family summaries copied."),()=>alert(text));
}
function buildPlanningReportHTML(){
  const text = buildPlanningStudioText();
  const prompt = buildPlanningPrompt();
  const lines = text.split("\n");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Teacher Data Dashboard Planning Report</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#102033;line-height:1.5}h1{font-size:30px;margin-bottom:4px}h2{margin-top:26px;border-bottom:2px solid #dbe5ef;padding-bottom:6px}.box{border:1px solid #dbe5ef;border-radius:12px;background:#f8fafc;padding:14px;margin:12px 0}pre{white-space:pre-wrap;border:1px solid #dbe5ef;border-radius:12px;padding:14px;background:#fbfdff}ul{padding-left:22px}@media print{body{margin:18px}}</style></head><body><h1>Teacher Data Dashboard Planning Report</h1><p>Generated ${escapeHtml(new Date().toLocaleString())}</p><div class="box"><strong>${escapeHtml(lines[1] || "Planning purpose")}</strong><br>${escapeHtml(lines[2] || "")}<br>${escapeHtml(lines[3] || "")}</div><h2>Planning Summary</h2><pre>${escapeHtml(text)}</pre><h2>AI Prompt Export</h2><pre>${escapeHtml(prompt)}</pre></body></html>`;
}
function downloadPlanningReportHTML(){
  if(!getFilteredRecords().length){ toast("No planning report to export yet."); return; }
  downloadText("data_studio_master_planning_report.html", buildPlanningReportHTML(), "text/html");
}
...
function renderActionPlan(){
  ...
  const trendLine = pred.reg ? `The current trend projects ${pct(pred.projected)} in ${pred.steps} assessment step(s), with ${pred.confidence.toLowerCase()} confidence.` : "Prediction needs at least two assessment windows; use the current average and priority standards for now.";
  return `
    <div class="container">
      ${renderFilters()}
      <section class="card soft">
        ...
        <div class="plan-grid">
          <div>
            ${[
              ["1","Name the focus",`Prioritize <strong>${escapeHtml(focus)}</strong>. ${priorities[0] ? `${pct(priorities[0].belowPct)} of selected records are below target on this standard.` : "A standard column will make this more precise."}`],
              ["2","Teach the shortest high-leverage move",`Use a 10–12 minute reteach: model one example, ask students to annotate the thinking, then complete one guided check with immediate feedback.`],
              ["3","Group for practice",`${groups.urgent.length} student(s) need intensive reteach, ${groups.strategic.length} need guided practice, ${groups.monitor.length} are on track/monitor, and ${groups.extension.length} are ready for extension.`],
              ["4","Collect quick evidence",`Use a 3-question exit ticket: one fluency item, one grade-level item, and one explain-your-thinking prompt. Re-run this dashboard with the exit ticket results.`]
            ].map(step=>`<div class="plan-step"><div class="step-number">${step[0]}</div><div><p class="step-title">${step[1]}</p><p class="step-text">${step[2]}</p></div></div>`).join("")}
          </div>
          <div class="segment">
            ...
            <div class="segment-body">
              ...
              <ul class="action-list">
                <li><strong>SPED/ESOL access move:</strong> Add a visual model, vocabulary bank, and sentence frame before independent practice.</li>
      ...
      <section class="grid grid-2">
        <div class="card">
          ...
          ${table(["Rank","Standard / Skill","Average","Below Target","Records"], priorities.map((p,i)=>[i+1,p.standard,pct(p.avg),pct(p.belowPct),p.n]))}
        </div>
        <div class="card">
          ...
          ${table(["Group","Count","Teacher Move"], [["Intensive",groups.urgent.length,"Model + guided practice + immediate check"],["Guided Practice",groups.strategic.length,"Scaffolded practice with feedback"],["Monitor",groups.monitor.length,"Grade-level task plus explanation"],["Extension",groups.extension.length,"Challenge/error analysis/student-created problem"]])}
        </div>
      </section>
    </div>
  `;
      ...
      <section class="card">
        ...
        <div class="student-card-grid">
          ${summaries.map(s=>{
            ...
            const lowStandards = standardSummaries(s.rows).slice(0,2).map(x=>x.standard).join(", ") || "General practice";
...
function renderWhatIf(){
  ...
  const changedTargetProf = scores.length ? scores.filter(s=>s>=Math.min(100,currentTarget+5)).length/scores.length*100 : NaN;
  const pred = predictionAnalysis(data);
  return `
    <div class="container">
      ${renderFilters()}
...
function renderGuide(){
  return `
    <div class="container">
      <section class="card soft">
        <div class="card-header"><div><h2 class="card-title">Teacher Guide</h2><p class="card-subtitle">A simple operating guide so Teacher Data Dashboard is usable by teachers with any level of data background.</p></div></div>
...
function copyActionPlan(){
  ...
  const pred = predictionAnalysis(data);
  const text = [
    "Teacher Data Dashboard — Action Plan",
    `Target: ${target}%`,
    `Priority focus: ${priorities[0]?.standard || "Not available"}`,
    priorities[0] ? `Why: average ${pct(priorities[0].avg)}, ${pct(priorities[0].belowPct)} below target.` : "Why: add standard/skill data for a clearer priority.",
    "",
    "Flexible Groups:",
    ...
    "",
    pred.reg ? `Prediction: ${pct(pred.projected)} projected next average (${pred.confidence} confidence).` : "Prediction: needs at least two assessment windows.",
    "",
    "Next evidence: 3-question exit ticket with one fluency item, one grade-level item, and one explain-your-thinking prompt."
  ].join("\n");
  navigator.clipboard?.writeText(text).then(()=>toast("Action plan copied."),()=>alert(text));
}
...
function exportCSV(){
  ...
  const rows = [["Student","Class/Period","Assessment","Standard","Score"]].concat(data.map(d=>[d.student,d.className,d.assessment,d.standard,d.score]));
  downloadText("data_studio_master_filtered_records.csv", rows.map(r=>r.map(csvEscape).join(",")).join("\n"), "text/csv");
}
...
function saveProject(){
  ...
  downloadText("data_studio_master_project.json", JSON.stringify(payload,null,2), "application/json");
}
function loadProjectFile(file){
  ...
  reader.onload = e => {
    try{
      ...
      if(!payload || !Array.isArray(payload.rawRows)) throw new Error("This does not look like a Data Studio Master project file.");
      ...
      state.extraHeaders = payload.extraHeaders || buildHeaders(state.extraRows);
...
function loadSample(){
  const students = ["Amara","Ben","Camila","Diego","Elena","Farah","Gabriel","Hana","Isaac","Janelle","Khalil","Lucia","Mateo","Noor","Omar","Priya"];
  const standards = ["6.EE.C.9 Variables","6.G.A.1 Area","6.G.A.2 Volume","6.SP.A.1 Statistical Questions"];
  const assessments = ["Unit 1 Check","Unit 2 Check","Unit 3 Check","Unit 4 Check"];
  const rows = [];
  students.forEach((student, si) => {
    ...
    assessments.forEach((assessment, ai) => {
      standards.forEach((standard, sti) => {
        ...
        const score = clamp(base + ai*5 - (sti===1?6:0) + noise, 28, 99);
        rows.push({
  ...
  state.extraRows = students.map((student, i) => ({
    ...
    "Language Level Band": i%3===0 ? "Entering/Emerging" : i%3===1 ? "Developing" : "Expanding/Bridging",
    "Intervention Group": i%4===0 ? "Small Group" : i%4===1 ? "Check-ins" : "Core Only"
  }));
...
function copyReport(){
  const data = getFilteredRecords();
  if(!data.length){ toast("No report to copy yet."); return; }
  const scores = data.map(d=>d.score);
  const standards = standardSummaries(data).slice(0,5);
  const pred = predictionAnalysis(data);
  const text = [
    "Teacher Data Dashboard Summary",
    `Records analyzed: ${data.length}`,
    `Students: ${unique(data.map(d=>d.student)).length}`,
    ...
    "",
    "Priority standards:",
    ...standards.map((s,i)=>`${i+1}. ${s.standard}: avg ${pct(s.avg)}, ${pct(s.belowPct)} below target`),
    "",
    pred.reg ? `Prediction: projected next average ${pct(pred.projected)} (${pred.confidence} confidence).` : "Prediction: needs at least two assessment windows.",
    "",
  ...
  navigator.clipboard?.writeText(text).then(()=>toast("Summary copied."),()=>alert(text));
}
function copyGroups(){
  ...
  navigator.clipboard?.writeText(text).then(()=>toast("Groups copied."),()=>alert(text));
}
function render(){
