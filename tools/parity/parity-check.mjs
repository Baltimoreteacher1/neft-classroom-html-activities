...
const IGNORED = [
  ...
  [
    (p) => p.startsWith("reports/") && !trackedReports().has(p),
    "gitignored audit outputs under reports/* are regenerated by local test/audit runs and absent from CI's clean checkout; the 12 tracked reports files still count",
  ],
  [
    (p) => p.startsWith("teacher-tools/canvas-scorm/packages/") || p.startsWith("scorm-packages/"),
    "gitignored SCORM package zips rebuilt locally by validate:scorm:fleet on every gate run; absent from CI's clean checkout, so they never reach production",
  ],
];

let trackedReportsCache = null;
function trackedReports() {
  if (!trackedReportsCache) {
    let out = "";
    try {
      out = execFileSync("git", ["ls-files", "reports"], { encoding: "utf8" });
    } catch {
      // outside a git checkout every reports/ file counts, the safe direction
    }
    trackedReportsCache = new Set(out.trim().split("\n").filter(Boolean));
  }
  return trackedReportsCache;
}
