...
const SPECS = [
  /* ---- UNIT 1 · The Pattern Engine (review bridge + volume intro) ---- */
  {
    ...
    gates: ["g1", "g2"],
    pages: [1, 2, 3, 4, 5],
    title: "Benchmark &amp; <em>Brass</em>",
    file: "axiom-city-u1-e1-benchmark-and-brass.html",
    lessons: "Review: Fractions &amp; Decimals",
    topic: "REVIEW · FRACTIONS &amp; DECIMALS",
    eplabel: "E1 · Benchmark &amp; Brass",
    epcovers: "½-benchmark fractions &middot; add/subtract decimals",
  },
  {
    slug: "u1e2",
    unit: 1,
    ...
    title: "Sequences &amp; <em>Cubes</em>",
    file: "axiom-city-u1-e2-sequences-and-cubes.html",
    lessons: "Patterns &amp; Volume (intro)",
    topic: "PATTERNS &amp; VOLUME",
    eplabel: "E2 · Sequences &amp; Cubes",
  ...
  /* ---- UNIT 7 · Below Zero (6.NOS.C.5–8, 6.GR.A.3) ---- */
  {
    ...
    gates: ["g1", "g2"],
    pages: [1, 2, 3, 4, 5],
    title: "Lights <em>Out</em>",
    file: "axiom-city-u7-e1-lights-out.html",
  ...
  },
  {
    ...
    gates: ["g3"],
    pages: [6, 7],
    title: "Signal in the <em>Snow</em>",
    file: "axiom-city-u7-e2-signal-in-the-snow.html",
    lessons: "Lesson 7-3",
  ...
  },
  {
    ...
    title: "The Grid <em>Map</em>",
    file: "axiom-city-u7-e3-the-grid-map.html",
  ...
  /* ---- UNIT 8 · The Balance Vault (6.AT.C.8–8) ---- */
  {
    ...
    gates: ["g1", "g2"],
    pages: [1, 2, 3, 4, 5],
    title: "The <em>Antechamber</em>",
    file: "axiom-city-u8-e1-the-antechamber.html",
  ...
  },
  {
    ...
    gates: ["g3"],
    pages: [6, 7],
    title: "The Gear <em>Locks</em>",
    file: "axiom-city-u8-e2-the-gear-locks.html",
    lessons: "Lesson 8-3",
    topic: "ONE-STEP × / ÷",
  ...
  },
  {
    ...
    gates: ["g4", "g5", "boss"],
    pages: [8, 9, 10, 11, 12, 13, 14],
    title: "The Fog <em>Gallery</em>",
    file: "axiom-city-u8-e3-the-fog-gallery.html",
  ...
  /* ---- UNIT 9 · Cause & Effect (6.AT.D.11) ---- */
  {
    ...
    gates: ["g1", "g2"],
    pages: [1, 2, 3, 4, 5],
    title: "Lines That <em>Talk</em>",
    file: "axiom-city-u9-e1-lines-that-talk.html",
  ...
  },
  {
    ...
    gates: ["g3", "g4", "g5", "boss"],
    pages: [6, 7, 8, 9, 10, 11, 12, 13, 14],
    title: "The Rule in the <em>Table</em>",
    file: "axiom-city-u9-e2-the-rule-in-the-table.html",
  ...
  /* ---- UNIT 10 · Boundless (capstone remix) ---- */
  {
    ...
    gates: ["g1", "g2", "g3"],
    pages: [1, 2, 3, 4, 5, 6, 7],
    title: "Festival to Night <em>Market</em>",
    file: "axiom-city-u10-e1-festival-to-night-market.html",
    lessons: "Capstone Remix · Part 1",
    topic: "STATS &middot; RATIOS &middot; PERCENT",
    eplabel: "E1 · Festival to Night Market",
    epcovers: "statistics, ratios &amp; percent remix &middot; Units 2–4",
  },
  {
    slug: "u10e2",
    unit: 10,
    ...
    gates: ["g4", "g5", "boss"],
    pages: [8, 9, 10, 11, 12, 13, 14],
    title: "Brightside to <em>Boundless</em>",
    file: "axiom-city-u10-e2-brightside-to-boundless.html",
    lessons: "Capstone Remix · Part 2",
    topic: "GEOMETRY &middot; EXPRESSIONS &middot; FINALE",
    eplabel: "E2 · Brightside to Boundless",
    epcovers: "area, volume, expressions &amp; distance remix &middot; Units 5–7 + finale",
  },
];
...
function buildIntroPage(spec, vmeta) {
  const lineup = ["#sym-maya", "#sym-theo", "#sym-zara", "#sym-dev"]
    .map((s, i) => `<g transform="translate(${20 + i * 92},6) scale(.74)"><use href="${s}"/></g>`)
    .join("");
  return {
...
function buildJournalPage(spec, vmeta, rows) {
  const items = rows
    .map((r) => `<div class="jrow"><b>${r[0]}</b><span>${r[1]}</span></div>`)
    .join("");
  return {
...
function buildContent(spec, data, helpers) {
  ...
  const stdRows = TEACHER.standards.filter(
    (r) => labels.some((l) => r[2].includes(l)) || /All gates|Hint ladders|All gates:/i.test(r[2]),
  );
...
function rewriteCover(cover, spec, vmeta) {
  ...
  c = c.replace(
    /<h1>[\s\S]*?<\/h1>/,
    `<h1>${spec.title.replace(/&amp;/g, "&").replace(" ", "<br>")}</h1>`,
  );
  c = c.replace(
    /<span class="vol">[\s\S]*?<\/span>/,
    `<span class="vol">UNIT ${spec.unit} · EP ${spec.ep} · ${spec.topic}</span>`,
  );
  c = c.replace(
    /<div id="coverBottom">[\s\S]*?<\/div>/,
    `<div id="coverBottom"><span>AN INTERACTIVE<br>GRAPHIC NOVEL EPISODE</span><span>GRADE 6 · UNIT ${spec.unit}<br>EDUWONDERLAB</span></div>`,
  );
  c = c.replace(
    /aria-label="Cover of[\s\S]*?">/,
    `aria-label="Cover of Axiom City Unit ${spec.unit}, Episode ${spec.ep}: ${spec.title.replace(/<\/?em>/g, "")}, part of the ${vmeta.title} arc.">`,
  );
  return c;
}
function rewriteHud(hud, spec) {
  return hud.replace(
    /<div class="brand">AXIOM CITY<small>[\s\S]*?<\/small><\/div>/,
...
function rewriteModals(modals, spec) {
  return modals.replace(
    /<h2 id="tmTitle">[\s\S]*?<\/h2>/,
    `<h2 id="tmTitle">Teacher Guide — U${spec.unit} · E${spec.ep}</h2>`,
  );
}
...
for (const spec of targets) {
  ...
  built++;
  console.log(
    `✓ ${spec.file}  (vol ${spec.vol}, gates ${spec.gates.join(",")}, ${html.length} bytes)`,
  );
}
