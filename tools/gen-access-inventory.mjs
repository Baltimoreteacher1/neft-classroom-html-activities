...
try {
  ...
  const v8 = window.ACCESS_LAB_V8,
    v9 = window.ACCESS_LAB_V9;
  let scenes = 0,
    patched = 0;
  for (const d of Object.values(DATA.domains))
    for (const L of Object.values(d.levels))
      for (const a of L.activities || []) {
        if (v8?.scenes?.[a.id] && !a.scene) {
          a.scene = v8.scenes[a.id];
          scenes++;
        }
        if (v9?.patches?.[a.id]) {
          Object.assign(a, v9.patches[a.id]);
          patched++;
        }
      }
  ...
  const out = {
    ...
    totals: {
      activities: total,
      v7Activities: v7count,
      scenesApplied: scenes,
      v9Patched: patched,
      tests: (DATA.tests || []).length,
      dataModules: modules.length,
    },
  ...
  console.log(
    `gen-access-inventory: ${total} activities (${v7count} v7, ${scenes} scenes, ${patched} patched) → inventory/config.json`,
  );
} catch (e) {
