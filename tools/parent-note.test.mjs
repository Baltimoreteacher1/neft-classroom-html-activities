...
import {
  ...
  buildNoteHTML,
  buildWeeklyDigest,
  buildWeeklyWarmupExitSummary,
  familyHomeworkFor,
  summarizeGrades,
} from "../scripts/lib/parent-note.mjs";
...
const evil = summarizeGrades({
  activities: ["<img src=x onerror=alert(1)>"],
  rows: [["Kid", "601", 40, "40"]],
});
...
assert.ok(
  dig.includes("/lessons/4-2/homework.html"),
  "5-minute activity links weakest lesson homework",
);
...
const evil2 = buildWeeklyDigest({
  studentName: "Kid",
  section: "601",
  activities: [
    {
      activityId: "x",
      activityTitle: "<img src=x onerror=alert(1)>",
      scorePct: 40,
      progressPercent: 10,
    },
  ],
...
assert.ok(
  weSummary.includes("🇺🇸 English") && weSummary.includes("🇲🇽 Español"),
  "warmup-exit summary bilingual",
);
...
const nullWarmup = buildWeeklyWarmupExitSummary({
  ...
  section: "605",
  weeklyLessons: [
    { lessonId: "1-1", lessonTitle: "Test", warmupPct: null, exitTicketCorrect: null, date: "Mon" },
  ],
});
