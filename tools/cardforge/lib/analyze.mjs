...
export function analyzeLesson(adapterResult) {
  ...
  if (!standard && topicStandard) {
    standard = topicStandard;
    uncertain.add("standard");
  }
  if (!topic) {
    missing.add("topic");
  }
  ...
  const ulMatch = text.match(
    /\b(?:unit\s*(\d{1,2}))?\D{0,12}lesson\s*(\d{1,2})(?:[.-](\d{1,2}))?/i,
  );
  if (ulMatch) {
  ...
  if (unit == null && standard) {
    ...
    if (fam && UNIT_BY_STANDARD_FAMILY[fam]) {
      unit = UNIT_BY_STANDARD_FAMILY[fam];
      uncertain.add("unit");
    }
  }
  if (unit == null) {
  ...
  // Formulas mentioned
  const formulas = uniq(
    (text.match(/\b[A-Za-z]\s*=\s*[^.;\n]{2,40}/g) || []).map((s) => s.trim()),
  ).slice(0, 6);

  const objective = a.objective || (topic ? `I can work with ${topic.replace(/-/g, " ")}.` : null);
  ...
  return {
    ...
    lesson,
    topic,
    gradeLevel: a.gradeLevel || "Grade 6",
    standard,
    objective,
    languageObjective: a.languageObjective || null,
    vocabulary: vocab,
    prerequisites: a.prerequisites || [],
    formulas,
    examples: a.examples || [],
    practiceTypes: a.practiceTypes || [],
    misconceptions: a.misconceptions || [],
    esolSupports: a.esolSupports || [],
    spedSupports: a.spedSupports || [],
    timeEstimate: a.timeEstimate || "~45 min",
    suggestedResources: uniq([
      "teacher-guide",
      "student-practice",
      "answer-key",
      "exit-ticket",
