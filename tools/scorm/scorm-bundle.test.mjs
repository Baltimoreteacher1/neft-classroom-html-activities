...
check("bundle names are teacher-readable and filesystem-safe", () => {
  for (const e of readZip(buildBundle(IDS)).filter((x) => x.name.endsWith(".zip"))) {
    ...
    // Runtime v2 naming: <id>_<Short_Title>_SCORM.zip. The point of the check
    // is unchanged — a teacher must be able to tell the files apart in a
    // Downloads folder without opening them — so the title fragment is
    // REQUIRED, not optional, and the name must stay cross-platform safe.
    assert.match(base, /^\d+-\d+_[A-Za-z0-9]+[A-Za-z0-9_]*_SCORM\.zip$/, `opaque name: ${base}`);
    assert.doesNotMatch(base, /eduwonderlab/i, `brand prefix is back: ${base}`);
