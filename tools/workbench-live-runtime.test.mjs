...
if (process.env.MWB_RUNTIME_TEST !== "1") {
  console.log("↷ Live Board runtime test skipped (set MWB_RUNTIME_TEST=1 with local Vite + Worker)");
  process.exit(0);
}
...
try {
  ...
  console.log("✓ Live Board rejects forged hosts, relays boards, and releases expired capabilities");
} finally {
  await browser.close();
}
