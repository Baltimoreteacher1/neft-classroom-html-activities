/**
 * End-to-end flow for the Unit 1 culminating project Version A —
 * /math/unit-1/projects/version-a/ ("Launch Day Supply Depot").
 *
...
const ROUTE = "/math/unit-1/projects/version-a/";
...
test.describe("Unit 1 culminating project Version A", () => {
  test("a student can work all three parts, and a wrong answer is diagnosed rather than answered", async ({
    ...
    await page.locator(".step-trail-item", { hasText: "Tables" }).click();
    ...
    await page.getByRole("button", { name: /Check my table order/i }).click();
    ...
    await page.getByRole("button", { name: /Check my beverage order/i }).click();
    ...
    await page.getByRole("button", { name: /Submit the block party order/i }).click();
  ...
  test("teacher class context does not segment or leak student project state", async ({ page }) => {
    ...
    await page.reload();
    await launchAtLevel(page, 1);
    await page.locator(".step-trail-item", { hasText: "Beverages" }).click();
    await expect(page.locator("#step-3")).toHaveClass(/active/);
