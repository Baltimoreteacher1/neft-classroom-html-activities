...
test.describe("AI Learning Hub - Browser QA Suite", () => {
  ...
  // Role cards are [student, teacher, ai101] since the Parent/Family card
  // was removed (2026-07-06) — teacher is nth(1).
  ...
  // ─── 21a. Tutor chat actually REPLIES (regression guard) ────────
  // The dead-chat bug (missing showTypingIndicator) passed every
  // presence-only test twice. This asserts real behavior: submit the
  // prompt, receive a bot message, answer, receive another bot message —
  // with zero uncaught page errors.
  test("Tutor chat replies to prompt submit and to an answer", async ({ page }) => {
    const errors = [];
    ...
    // First tutor question must appear (typing indicator delay is 1200ms)
    const botBubble = page.locator("#chat-log .chat-row.bot .chat-bubble");
    await expect(botBubble.first()).toBeVisible({ timeout: 5000 });
    const firstReply = await botBubble.first().textContent();
    expect(firstReply.length).toBeGreaterThan(20);

    // Answering (even wrongly) must produce a SECOND tutor reply
    await page.locator("#chat-input").fill("999999");
    await page.locator(".chat-send-btn").click();
    await expect(botBubble.nth(1)).toBeVisible({ timeout: 8000 });

    expect(errors).toEqual([]);
  });

  // ─── 21b. Lesson picker shows the unit's 5 real lessons ─────────
  test("Lesson picker lists 5 lessons and switching updates the problem", async ({ page }) => {
    const errors = [];
    page.on("pageerror", (error) => errors.push(error.message));
    ...
    const lessonChips = page.locator("div.builder-col:nth-child(2) .builder-chip");
    await expect(lessonChips).toHaveCount(5);

    const before = await page.locator("#current-math-problem").textContent();
    await lessonChips.nth(1).click();
    await page.waitForTimeout(300);
    const after = await page.locator("#current-math-problem").textContent();
