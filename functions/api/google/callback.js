...
export async function onRequestGet({ request, env }) {
  if (!isConfigured(env))
    return json({ ok: false, error: "Google sync is not configured." }, { status: 503 });
  ...
  if (error)
    return Response.redirect(
      `${appBaseUrl}/noam-school-v10/?google=error&reason=${encodeURIComponent(error)}`,
      302,
    );
  if (!code || !state)
    return json({ ok: false, error: "Missing Google OAuth code or state." }, { status: 400 });

  const session = await loadSession(request, env);
  if (!session || session.data.oauth_state !== state)
    return json({ ok: false, error: "Google OAuth state check failed." }, { status: 400 });
