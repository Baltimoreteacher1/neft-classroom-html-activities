...
export async function onRequestGet({ request, env }) {
    ...
    return json(
      {
        ...
        configured: false,
        connected: false,
        error: "Google sync is not configured yet.",
      },
      { status: 503 },
    );
  }
  ...
  if (!session?.data?.access_token) {
    return json(
      { ok: false, configured: true, connected: false, error: "Google is not connected." },
      { status: 401 },
    );
  }
  ...
  await env.NOAM_SCHOOL_KV.put(`snapshot:${session.id}`, JSON.stringify(snapshot), {
