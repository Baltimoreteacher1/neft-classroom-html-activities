    ...
    closeBundle() {
      ...
      if (existsSync(lessonsDir)) {
        ...
        for (const dir of readdirSync(lessonsDir, { withFileTypes: true })) {
          ...
          // Per-lesson practice worksheets and their answer keys, linked from
          // the curriculum hub. Set A is the lesson's tiered practice; Set B
          // ("-2") is the second form built from the authored problems Set A
          // does not print — see scripts/lib/worksheet-set-b.mjs.
          for (const page of [
            "worksheet.html",
            ...
            const destDir = resolve(__dirname, "dist", "lessons", dir.name);
            mkdirSync(destDir, { recursive: true });
            cpSync(src, resolve(destDir, page));
          }
          // Small-group Practice Set — the continuation packet generated by
          // scripts/generate-sg-practice.mjs, linked from the curriculum hub
          // beside each small-group lesson's worksheet.
          for (const page of ["practice.html", "practice-answer-key.html"]) {
            const src = resolve(lessonsDir, dir.name, page);
            if (!existsSync(src)) continue;
            const destDir = resolve(__dirname, "dist", "lessons", dir.name);
            mkdirSync(destDir, { recursive: true });
