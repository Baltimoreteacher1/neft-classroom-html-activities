 ...
 *   2. Auto-wraps window.NTResults.finish() — the shared assessment kit's
 *      completion hook (teacher-tools/nt-results.js) — so ANY activity that
 *      already calls finish() also reports a gradebook event. No per-activity
 *      scoring code changes are required.
 ...
 *   - Co-exists with nt-sync.js: both wrap NTResults.finish independently and
 *     chain through to the original. Different backends, no conflict.
 *
...
(function (global) {
  ...
  function resolveIdentity(typedName) {
  ...
  function storedName() {
    try {
      // Reuse identity other Neft layers may already have captured.
      return (
  ...
  function activityId() {
    // Stable id from the folder path, e.g. /math/unit2/index.html -> math-unit2
    try {
  ...
  /* Core send used by both the auto-wrap and the public API. */
  function send(payload) {
    try {
      if (!configured(bridge.ingestKey)) return; // inert until key pasted
      payload = payload || {};
      var id = resolveIdentity(payload.studentName);
      ...
      // Deterministic eventId => identical final submissions collapse server-side.
      var dedupeKey =
        aId +
        "|" +
        id.studentId +
  ...
  /* Re-attempt wrap at several lifecycle points (NTResults may load late). */
  wrapNTResults();
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", wrapNTResults);
  }
  global.addEventListener("load", wrapNTResults);
