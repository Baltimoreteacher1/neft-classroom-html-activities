...
export function buildTptSlideDeckV2(ctx) {
  ...
  add(
    ...
    { type: "title", notes: "Display while students enter. Start presenter mode with ▶ Present." },
  );
  ...
  add(
    ...
    { type: "learning-targets", notes: "Post objectives on board. Return to these at closure." },
  );
  ...
  add(
    ...
    { type: "notice-wonder", section: "launch" },
  );
  ...
  // Real drag sort
  const dragHtml = buildRealDragSortSlide(
    dragSort,
    themeEmoji,
    themeName,
    contentObj,
    lessonId.replace(/[^a-z0-9]/gi, ""),
  );
  ...
  add(
    "⚠️ Error",
    "Error Analysis",
    `${learningHeader(contentObj)}${slideHeader(themeEmoji, themeName, "Find the Mistake")}${buildErrorAnalysisSlide(practiceHtml)}`,
    { type: "error-analysis", section: "practice", notes: data.practice?.commonMistake || "" },
  );
  ...
  add(
    "📝 Exit",
    "Exit Ticket",
    `${learningHeader(contentObj)}${slideHeader(themeEmoji, themeName, "Exit Ticket")}<div class="slide-card">${exitTicketHtml}</div>`,
  ...
  add(
    ...
    buildExitTicketOpen(
      exitOpenStem ||
        `Explain the key idea from today's lesson in your own words. Use at least one vocabulary word.`,
      themeEmoji,
      themeName,
      contentObj,
    ),
