...
function buildVocabExamplesStrip(vocabList) {
  const withExamples = (vocabList || []).filter(
    (v) => Array.isArray(v.examples) && v.examples.length,
  );
...
function buildTurnAndTalkSlide(
  talk,
  title,
  themeEmoji,
  themeName,
  ...
  const wordBank = (talk.wordBank || [])
    .map(
      (w) => `<span class="vocab-pill" onclick="insertAtCursor(this.textContent)">${esc(w)}</span>`,
    )
    .join("");
...
function buildChoiceBoardSlide(_contentObj, _themeEmoji, _themeName, vocabList, keyIdea) {
  ...
  const choices = [
    {
      icon: "✏️",
      title: "✏️ Draw & Label",
      desc: `Sketch a model for today's problem. Label it using ${labelList}.`,
    },
    {
      icon: "💬",
      title: "💬 Explain It",
  ...
  const side = `${refTeacherNote("students", "Pick ONE numbered option, then do it in the workspace right on this slide.")}
    ...
    ${
      terms.length
        ? `<p class="ref-side-prompt"><strong>Use these words:</strong> ${terms
            .slice(0, 4)
            .map((t) => esc(t))
            .join(", ")}</p>`
        : ""
...
function buildThinkWriteSlide(
  ...
  const frames = [
    ...
    {
      title: "Because / But / So",
      prompt: `Because ${t1} means ___, but a tricky part is ___, so I have to ___.`,
    },
    {
    ...
    {
      title: "Prove It",
      prompt: `I can prove my answer is correct by ___, using ${t2} to check my work.`,
    },
  ];
...
function buildBecauseButSoSlide(_contentObj, _themeEmoji, _themeName, keyIdea, vocabList = []) {
  ...
  const kernel =
    ...
    `${t1.charAt(0).toUpperCase()}${t1.slice(1)} matters in this lesson`;
  const wordBank = terms
    .slice(0, 5)
    .map(
      (w) => `<span class="vocab-pill" onclick="insertAtCursor(this.textContent)">${esc(w)}</span>`,
    )
  ...
  const cols = [
    {
      ...
      cls: "twr-bbs-because",
      job: "Give a reason",
      prompt: `${esc(kernel)} because ___`,
    },
    {
      conj: "but",
      cls: "twr-bbs-but",
      job: "Name a tricky part",
      prompt: `${esc(kernel)} but ___`,
    },
...
function buildSentenceExpansionSlide(
  ...
  const starterHtml = starters
    .map(
      (s) =>
        `<span class="twr-starter-chip" onclick="insertAtCursor(this.textContent.replace(/[…\\u2026].*$/,'').trim()+' ')">${esc(s)}</span>`,
    )
    .join("");
  const main = `
...
function buildGoalTrackerSlide(contentObj, _themeEmoji, _themeName) {
  const levels = [
    { num: 1, label: "Not Yet", desc: "I need more help. The idea does not make sense to me yet." },
    {
      num: 2,
      label: "Getting There",
      desc: "I understand the idea but I make mistakes when I work.",
    },
    {
      num: 3,
      label: "Got It!",
      desc: "I can solve problems independently and explain my thinking.",
    },
    {
      num: 4,
      label: "Teaching Others",
      desc: "I can explain the strategy to a classmate clearly.",
    },
  ];
...
function buildUnitActivitySlide(
  variant,
  contentObj,
  themeEmoji,
  themeName,
  ...
  else if (variant === "error-analysis")
    return `${learningHeader(contentObj)}${slideHeader(themeEmoji, themeName, "Find the Mistake")}${practiceHtml || ""}`;
  else
    widget = `<p class="card-desc">Complete the interactive activity using today's strategy.</p>`;
...
export function buildTptSlideDeckV3(ctx) {
  ...
  const maybeCfu = (prompt) => {
    if (slidesSinceCfu >= 4) {
      cfuCount += 1;
      add("✋ CFU", `CFU ${cfuCount}`, buildCfuSlide(cfuCount, themeEmoji, themeName, prompt), {
        type: "cfu",
        section: "check",
      });
      slidesSinceCfu = 0;
  ...
  add(
    ...
    }),
    {
      type: "title",
      section: "launch",
      notes: "Display while students enter. Start presenter mode with ▶ Present.",
    },
  );
  ...
  add(
    ...
    { type: "how-to", notes: "First day with deck? Spend 30 sec here. Skip on repeat lessons." },
  );
  ...
  `,
    ...
    ),
    {
      ...
      slideTitle: "Learning Targets",
      notes: "Post objectives on board. Return to these at closure.",
    },
  );
  ...
  add(
    ...
    { type: "agenda", notes: "Adjust minutes based on your block schedule." },
  );

  // 5 Section: Launch
  add("🚀 Launch", "Launch Section", buildSectionDivider("LAUNCH", 10, themeEmoji), {
    type: "section",
    section: "launch",
  });
  ...
  `,
    ...
    ${refTeacherNote("time", "3 min")}`,
    ),
    {
      type: "hook",
      section: "launch",
      slideTitle: "Warm-Up Hook",
      notes: talks[0]?.listenFor || "Listen for prior knowledge connections.",
    },
  );
  ...
  add(
    ...
    { type: "concept", section: "launch", notes: conceptKeyIdea || "" },
  );
  ...
  add(
    "👁️ I Do",
    "I Do — Watch Me",
    buildRevealSteps(iDoLines, "ido", esc(iDoTitle), themeEmoji, themeName, contentObj),
    {
      type: "worked-example",
      section: "instruction",
      notes: "Reveal one step at a time. Do NOT show all steps at once.",
    },
  );
  ...
  add(
    ...
    "I Do — Key Step",
    buildRevealSteps(
      iDoLines.slice(-2).length ? iDoLines.slice(-2) : iDoLines.slice(0, 2),
      "ido2",
      "I Do — Focus Step",
      themeEmoji,
      themeName,
    ...
    ),
    {
      type: "worked-example",
      section: "instruction",
      notes: "Second reveal sequence — focus on the most critical step.",
    },
  );
  ...
  add(
    "🤝 We Do",
    "We Do — Together",
    buildRevealSteps(weDoLines, "wedo", esc(weDoTitle), themeEmoji, themeName, contentObj),
    {
      type: "worked-example",
      section: "instruction",
      notes: "Call on students to predict each step before revealing.",
    },
  );
  ...
  add(
    ...
    { type: "you-do", section: "instruction" },
  );

  // Section: Vocabulary
  add("📚 Vocab", "Vocabulary Section", buildSectionDivider("VOCABULARY", 8, "📚"), {
    type: "section",
    section: "vocabulary",
  });
  ...
  if (vocabList.length) {
    ...
    add(
      ...
      ),
      {
        type: "vocab-card",
        section: "vocabulary",
        slideTitle: "Vocabulary",
        notes:
          "Post vocabulary on board for reference. Use the example / non-example row to surface misconceptions.",
      },
    );
  }
  ...
  if (vocabMatch)
    add("✅ Vocab", "Vocab Check", vocabMatch, { type: "vocab-card", section: "vocabulary" });

  const vocabCloze = buildVocabClozeSlide(vocabList, themeEmoji, themeName, contentObj);
  if (vocabCloze)
    add("❓ Vocab", "Which Word?", vocabCloze, { type: "interactive-mc", section: "vocabulary" });

  maybeCfu("Use one vocabulary word in a sentence about today's topic.");

  // Turn & Talk Launch with timer
  const ttLaunch = buildTurnAndTalkSlide(
    talks.find((t) => t.phase === "launch") || talks[0],
    "Turn &amp; Talk — Launch",
    themeEmoji,
    themeName,
    contentObj,
  ...
  if (ttLaunch)
    add("🗣️ Discuss", "Turn & Talk", ttLaunch, {
      type: "turn-talk-timer",
      section: "discuss",
      notes: talks.find((t) => t.phase === "launch")?.listenFor || "",
    });

  // Section: Explore
  add("🔍 Explore", "Explore Section", buildSectionDivider("EXPLORE &amp; PRACTICE", 18, "🔍"), {
    type: "section",
    section: "explore",
  });
  ...
  add(
    ...
    { type: "worked-example", section: "explore" },
  );
  ...
  add(
    ...
    { type: "explore", section: "explore", notes: exploreInstructions },
  );

  add("🖊️ CFU", "Whiteboard CFU", buildWhiteboardCfu(themeEmoji, themeName, contentObj), {
    type: "cfu",
    section: "check",
  });
  slidesSinceCfu = 0;

  const ttExplore = buildTurnAndTalkSlide(
    talks.find((t) => t.phase === "explore"),
    "Turn &amp; Talk — Explore",
    themeEmoji,
    themeName,
    contentObj,
  ...
  if (ttExplore)
    add("🗣️ Explore", "Discuss Explore", ttExplore, { type: "turn-talk-timer", section: "discuss" });

  // Practice MC
  if (mcProblems[0]) {
    const body = buildMcSlide(
      mcProblems[0],
      "mc1",
      "Practice Check A",
      themeEmoji,
      themeName,
      contentObj,
      "Give students 1 minute. Cold-call one student to defend their answer.",
    );
    if (body)
      add("📝 Practice", "Practice A", body, { type: "interactive-mc", section: "practice" });
  }
  if (mcProblems[1]) {
    const body = buildMcSlide(
      mcProblems[1],
      ...
      themeName,
      contentObj,
      "Partner discussion first, then vote.",
    );
    if (body)
      add("📝 Practice", "Practice B", body, { type: "interactive-mc", section: "practice" });
  }
  ...
  const primaryActivity =
    activityPlan.primary === "error-analysis"
      ? buildStructuredErrorSlide(data, themeEmoji, themeName, contentObj)
      : buildUnitActivitySlide(
          activityPlan.primary,
          contentObj,
          themeEmoji,
          themeName,
          vocabList,
  ...
  if (primaryActivity) {
    ...
    add(pMeta.icon || "🎯", pMeta.label || "Activity", primaryActivity, {
      type: activityPlan.primary,
      section: "practice",
    });
  }
  ...
  if (activityPlan.secondary === "drag-sort-ratios" || dragSort) {
    ...
    const categories = dragSort?.categories || [
    ...
    const buckets = categories
      .map((c, i) => refSortBucket(c.label || c, colorClasses[i % 3], c.id || c.label || c))
      .join("");
    ...
    add("↕️ Sort", "Sort It Out", sortMain, {
      type: "drag-sort",
      section: "practice",
      slideTitle: "Sort It Out",
    });
  }
  ...
  add(
    "🎯 Choice",
    "Choice Board",
    buildChoiceBoardSlide(contentObj, themeEmoji, themeName, vocabList, conceptKeyIdea),
    { type: "choice-board", section: "practice" },
  );
  ...
  add(
    "✍️ TWR",
    "Think Write",
    buildThinkWriteSlide(contentObj, themeEmoji, themeName, conceptKeyIdea, vocabList, data),
    { type: "think-write", section: "practice" },
  );
  ...
  add(
    "✍️ B/B/S",
    "Because But So",
    buildBecauseButSoSlide(contentObj, themeEmoji, themeName, conceptKeyIdea, vocabList),
    {
      type: "twr-conjunctions",
      section: "practice",
      slideTitle: "Because · But · So",
      notes:
        "TWR conjunctions. Cold-call one student per conjunction and chart the differences in reasoning.",
    },
  );
  ...
  add(
    "🌱 Expand",
    "Sentence Expansion",
    buildSentenceExpansionSlide(contentObj, themeEmoji, themeName, conceptKeyIdea, vocabList),
    {
      type: "twr-expansion",
      section: "practice",
      slideTitle: "Sentence Expansion",
      notes:
        "TWR sentence expansion. Model one expansion aloud first, then release to independent writing.",
    },
  );
  ...
  add(
    "📊 Workspace",
    "Student Workspace",
    buildStudentWorkspaceSlide(data.explore || {}, themeEmoji, themeName, contentObj),
    { type: "student-workspace", section: "practice" },
  );

  // Differentiation
  add("🎯 Diff", "Differentiation", buildDifferentiationSlide(data, themeEmoji, themeName), {
  ...
  add(
    ...
    { type: "partner-activity", section: "practice" },
  );
  ...
  add(
    ...
    { type: "real-world", section: "connect" },
  );

  const ttConnect = buildTurnAndTalkSlide(
    talks.find((t) => t.phase === "connect"),
    "Turn &amp; Talk — Connect",
    themeEmoji,
    themeName,
    contentObj,
  ...
  add("📝 Exit", "Exit Ticket", refExitSplit(exitReflection, exitQuick), {
    type: "exit-ticket",
    section: "closure",
  ...
  add("🏆 Goal", "Goal Tracker", buildGoalTrackerSlide(contentObj, themeEmoji, themeName), {
    type: "goal-tracker",
    section: "closure",
  });
  ...
  add(
    ...
    { type: "exit-ticket", section: "closure" },
  );
  ...
  add(
    ...
    { type: "activity-link", section: "closure" },
  );
  ...
  add(
    ...
    { type: "teacher-notes", notes: "Hide during presentation — use Notes panel (N key)." },
  );
  ...
  add(
    ...
    { type: "teacher-notes", notes: "CONFIDENTIAL — do not display to students." },
  );
