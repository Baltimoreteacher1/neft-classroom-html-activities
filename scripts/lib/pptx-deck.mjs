...
function panel(slide, pptx, x, y, w, h, fill = T.white, lineColor = T.line, radius = 0.08) {
  slide.addShape(pptx.ShapeType.roundRect, {
    ...
    shadow: { type: "outer", blur: 4, offset: 1, angle: 90, color: "1C2E42", opacity: 0.1 },
  });
}
...
function header(slide, pptx, c, title) {
  ...
  // footer
  slide.addText(`Reveal Math Grade 6  ·  Unit ${c.unit}  ·  Lesson ${c.lessonId}`, {
    x: 0.3,
    y: H - 0.32,
    w: 7,
    h: 0.28,
...
function writeLines(slide, pptx, x, y, w, count, gap = 0.3) {
  for (let i = 0; i < count; i++) {
    slide.addShape(pptx.ShapeType.line, {
      ...
      w,
      h: 0,
      line: { color: "C7CDD2", width: 0.75, dashType: "dash" },
    });
  }
}
...
function slideCover(slide, pptx, c) {
  ...
  slide.addText(`Unit ${c.unit}  ·  Lesson ${c.lessonId}`, {
    x: 1.85,
    y: 4.55,
    w: 2.1,
    h: 0.34,
  ...
  slide.addText("My Math Notebook", {
    x: 4.65,
    y: 0.6,
    w: 4.8,
    h: 0.4,
  ...
  slide.addText(
    [
      ...
      { text: "/ Yo puedo…", options: { italic: true, color: T.gray, fontSize: 9 } },
    ],
    { x: 4.6, y: 2.3, w: 4.85, h: 0.3, fontFace: HEAD, fontSize: 11 },
  );
  slide.addText(c.contentObj, {
    x: 4.6,
    y: 2.62,
    w: 4.85,
    h: 0.78,
  ...
  slide.addText(
    [
      { text: "I WILL EXPLAIN…  ", options: { bold: true, color: c.accent } },
      { text: "/ Objetivo de lenguaje", options: { italic: true, color: T.gray, fontSize: 9 } },
    ],
    { x: 4.6, y: 3.7, w: 4.85, h: 0.3, fontFace: HEAD, fontSize: 11 },
  );
  slide.addText(c.langObj, {
    x: 4.6,
    y: 4.02,
    w: 4.85,
    h: 0.92,
...
function slideBeCurious(slide, pptx, c) {
  ...
  slide.addText(c.launchNarrative, {
    x: 0.65,
    y: 1.65,
    w: 4.15,
    h: 1.7,
  ...
  slide.addText("Jot your first idea:", {
    x: 0.65,
    y: 3.45,
    w: 4,
    h: 0.25,
  ...
  const notice = (
    c.noticeStems.length
      ? c.noticeStems
      : ["I notice the numbers are ___.", "I notice ___ happens when ___."]
  ).slice(0, 3);
  slide.addText(
    notice.map((s) => ({ text: `•  ${s}`, options: { breakLine: true } })),
    {
      x: 5.35,
      y: 1.15,
      w: 4.1,
  ...
  const wonder = (
    c.wonderStems.length
      ? c.wonderStems
      : ["I wonder how ___ changes when ___.", "I wonder why ___."]
  ).slice(0, 3);
  slide.addText(
    wonder.map((s) => ({ text: `•  ${s}`, options: { breakLine: true } })),
    {
      x: 5.35,
      y: 3.4,
      w: 4.1,
...
function slideVocabulary(slide, pptx, c) {
  ...
  c.vocab.slice(0, 5).forEach((v, i) => {
    ...
    rows.push([
      {
        text: [
          { text: esc(v.term), options: { bold: true, color: T.navy } },
          ...(v.termEs
            ? [
                {
                  text: `\n${esc(v.termEs)}`,
                  options: { italic: true, color: T.gray, fontSize: 8.5 },
                },
              ]
            : []),
        ],
        options: { fill: { color: bg }, valign: "middle", fontSize: 9.5, fontFace: BODY },
      },
      {
        text: [
          { text: esc(v.definition), options: { color: T.body } },
          ...(v.definitionEs
            ? [
                {
                  text: `\n${esc(v.definitionEs)}`,
        ...
        options: { fill: { color: bg }, valign: "middle", fontSize: 9, fontFace: BODY },
      },
      {
        text: esc(v.visual || (v.examples && v.examples[0] && v.examples[0].text) || ""),
        options: {
          fill: { color: bg },
          valign: "middle",
          color: T.body,
  ...
  if (exTerm) {
    ...
    slide.addText(`${esc(exTerm.term)} — example vs. non-example:`, {
      x: 0.45,
      y: y - 0.02,
      w: 9.2,
      h: 0.26,
    ...
    exTerm.examples.slice(0, 4).forEach((ex, i) => {
      ...
      slide.addText(
        [
          { text: `${yes ? "✓" : "✗"} `, options: { bold: true, color: yes ? T.teal : T.red } },
          { text: `${esc(ex.text)}  `, options: { bold: true, color: T.navy } },
          { text: esc(ex.why), options: { color: T.body, fontSize: 8 } },
        ],
        { x: x + 0.12, y: yy, w: 4.26, h: 0.5, fontFace: BODY, fontSize: 9, valign: "middle" },
      );
    });
  }
}
...
function slideGuidedPractice(slide, pptx, c) {
  ...
  slide.addText(c.conceptHeading, {
    x: 0.65,
    y: 1.3,
    w: 5.6,
    h: 0.3,
  ...
  slide.addText("Your turn — show your work:", {
    x: 0.65,
    y: 4.25,
    w: 5.6,
    h: 0.25,
  ...
  slide.addText(esc(c.talk1 || "Explain each step to your partner using today's vocabulary."), {
    x: 6.8,
    y: 1.2,
    w: 2.7,
    h: 1.6,
...
function slideSortItOut(slide, pptx, c) {
  ...
  panel(slide, pptx, 0.45, 0.75, 6.0, 4.4, T.white, T.line);
  slide.addText(esc(c.sortInstructions), {
    x: 0.65,
    y: 0.9,
    w: 5.6,
    h: 0.4,
  ...
  slide.addText("Card Bank — cut or sort these cards:", {
    x: 0.65,
    y: 1.35,
    w: 5.6,
    h: 0.24,
  ...
  const items = c.sortItems.length
    ? c.sortItems.map((it) => it.text)
  ...
  cats.slice(0, 3).forEach((cat, i) => {
    ...
    slide.addText(esc(cat.label || cat), {
      x: bx,
      y: after + 0.1,
      w: bw,
      h: 0.32,
  ...
  panel(slide, pptx, 6.6, 0.75, 3.05, 4.4, T.tealLight, T.teal);
  sectionLabel(slide, 6.6, 0.75, 3.05, "💬 Talk It Over", T.teal);
  slide.addText(esc(c.talk2 || "Which cards were tricky to place? Explain how you decided."), {
    x: 6.8,
    y: 1.2,
    w: 2.7,
    h: 1.5,
...
function slideErrorAnalysis(slide, pptx, c) {
  ...
  slide.addText(esc(c.error.title), {
    x: 2.5,
    y: 0.92,
    w: 3.7,
    h: 0.3,
  ...
  slide.addText(
    "A classmate turned in the work below. One step has a mistake — find it, name it, fix it.",
    { x: 0.65, y: 1.3, w: 5.6, h: 0.4, fontFace: BODY, fontSize: 9.5, color: T.body },
  );
  // student work box
  panel(
    slide,
    ...
    1.78,
    5.65,
    0.3 + c.error.steps.length * 0.42 + 0.1,
    "F4F1E8",
    T.line,
    0.05,
  );
  slide.addText("Student's work (contains an error):", {
    x: 0.78,
    y: 1.85,
    w: 5.4,
    h: 0.24,
  ...
  c.error.steps.slice(0, 6).forEach((s, i) => {
    ...
    slide.addText(
      [
        { text: `${i + 1}. `, options: { bold: true, color: T.navy } },
        { text: `${esc(s.label)}:  `, options: { bold: true, color: T.navy } },
        { text: esc(s.work), options: { fontFace: "Courier New", bold: true, color: T.body } },
      ],
      { x: 0.85, y: yy, w: 5.35, h: 0.38, fontFace: BODY, fontSize: 9.5, valign: "middle" },
    );
  });
  const fy = 1.78 + 0.3 + c.error.steps.length * 0.42 + 0.25;
  slide.addText("Which step has the error? Circle it above.", {
    x: 0.65,
    y: fy,
    w: 5.6,
    h: 0.25,
  ...
  slide.addText(
    [
      { text: "The mistake was ", options: {} },
      { text: "___", options: { bold: true } },
      { text: " because ", options: {} },
      { text: "___", options: { bold: true } },
      { text: ".", options: {} },
    ],
    { x: 6.8, y: 1.2, w: 2.7, h: 0.55, fontFace: BODY, fontSize: 10, color: T.navy, valign: "top" },
  );
  writeLines(slide, pptx, 6.8, 1.95, 2.65, 3, 0.32);
  slide.addText("Fix it — rewrite the step correctly:", {
    x: 6.8,
    y: 3.05,
    w: 2.7,
    h: 0.3,
...
function slideTwoColumn(slide, pptx, c) {
  ...
  slide.addText("Work one problem here, matching each step on the left:", {
    x: 5.35,
    y: 1.25,
    w: 4.1,
    h: 0.4,
...
function slideChoiceBoard(slide, pptx, c) {
  ...
  slide.addText("Choose ONE to show what you know:", {
    x: 0.45,
    y: 0.7,
    w: 9.2,
    h: 0.3,
  ...
  const opts = [
    {
      icon: "✏️",
      title: "Draw & Label",
      desc: `Sketch a model for today's problem. Label it using ${t1}${t2 !== t1 ? ` and ${t2}` : ""}.`,
      bg: T.tealLight,
    },
    {
      icon: "💬",
      title: "Explain It",
      desc: c.keyIdea
        ? `In 2–3 sentences, explain "${c.keyIdea}" in your own words.`
        : "Explain today's key idea in your own words.",
      bg: T.amberLight,
    },
    {
      icon: "🧮",
      title: "Solve It",
      desc: "Work one practice problem and show every step clearly.",
      bg: T.coral,
    },
    {
      icon: "🎯",
      title: "Create a Problem",
      desc: `Write your own problem that uses ${t1}, then solve it and make an answer key.`,
      bg: "EAF2F1",
    },
  ];
  opts.forEach((o, i) => {
...
function slideIndependentPractice(slide, pptx, c) {
  ...
  c.practice.slice(0, 3).forEach((p, i) => {
    ...
    slide.addText(
      [
        ...
        { text: esc(p), options: { color: T.navy } },
      ],
      { x: 0.65, y: yy, w: 5.65, h: 0.5, fontFace: BODY, fontSize: 10.5, valign: "top" },
    );
  ...
  slide.addText(
    esc(
      c.talk3 ||
        c.talk1 ||
        "Compare answers with a partner. Where did you agree or disagree, and why?",
    ),
    { x: 6.8, y: 1.2, w: 2.7, h: 1.7, fontFace: BODY, fontSize: 10, color: T.navy, valign: "top" },
  );
...
function slideThinkWrite(slide, pptx, c) {
  ...
  slide.addText("Use evidence from today's lesson to complete each frame.", {
    x: 0.45,
    y: 0.66,
    w: 9.2,
    h: 0.3,
  ...
  const frames = [
    {
      t: "Explain the Rule",
      p: c.keyIdea
        ? `Today's key idea is "${c.keyIdea}" — and it works because ___.`
        : "The most important rule from today is ___, and it works because ___.",
    },
    {
      t: "Because / But / So",
      p: `Because ${t1} means ___, but a tricky part is ___, so I have to ___.`,
    },
    {
      t: c.error.steps.length ? "Catch the Mistake" : "Justify Your Method",
      p: c.error.steps.length
  ...
  frames.forEach((f, i) => {
    ...
    slide.addText(
      [
        { text: `Frame ${i + 1} — `, options: { color: T.gray } },
        { text: f.t, options: { bold: true, color: c.accent } },
      ],
      { x: 0.65, y: y + 0.1, w: 8.8, h: 0.3, fontFace: HEAD, fontSize: 11 },
    );
    slide.addText(f.p, {
      x: 0.65,
      y: y + 0.42,
...
function slideExitTicket(slide, pptx, c) {
  ...
  slide.addText("Today I learned that ___ because ___.", {
    x: 0.65,
    y: 1.2,
    w: 4.15,
    h: 0.4,
  ...
  slide.addText("One thing I am still not sure about is ___.", {
    x: 0.65,
    y: 3.1,
    w: 4.15,
    h: 0.4,
  ...
  slide.addText(
    [
      { text: "Tier 1 — ", options: { bold: true, color: c.accent } },
      { text: esc(c.exit.tier1), options: { color: T.navy } },
    ],
    { x: 5.35, y: 1.2, w: 4.1, h: 0.7, fontFace: BODY, fontSize: 10.5, valign: "top" },
  );
  (c.exit.choices.length ? c.exit.choices : ["A. ___", "B. ___", "C. ___"])
    .slice(0, 4)
    .forEach((ch, i) => {
      slide.addText(`${String.fromCharCode(65 + i)}.  ${esc(ch).replace(/^[A-D]\.\s*/, "")}`, {
        x: 5.5,
        y: 1.95 + i * 0.34,
        w: 3.9,
        h: 0.3,
  ...
  slide.addText(
    [
      { text: "Tier 2 — ", options: { bold: true, color: c.accent } },
      { text: esc(c.exit.tier2), options: { color: T.navy } },
    ],
    { x: 5.35, y: ty, w: 4.1, h: 0.55, fontFace: BODY, fontSize: 10.5, valign: "top" },
  );
...
function slideGoalTracker(slide, pptx, c) {
  ...
  slide.addText(
    [
      { text: "My Goal:  ", options: { bold: true, color: T.amber } },
      { text: esc(c.contentObj), options: { color: T.white } },
    ],
    { x: 0.7, y: 0.72, w: 8.7, h: 0.55, fontFace: BODY, fontSize: 11, valign: "middle" },
  );
  const levels = [
    { n: 1, l: "Not Yet", d: "I need more help. This does not make sense to me yet.", bg: T.coral },
    {
      n: 2,
      l: "Getting There",
      d: "I understand the idea but I make mistakes when I work.",
      bg: T.amberLight,
    },
    {
      n: 3,
      l: "Got It!",
      d: "I can solve problems on my own and explain my thinking.",
      bg: T.tealLight,
    },
    {
      n: 4,
      l: "Can Teach It",
      d: "I can clearly teach this strategy to a classmate.",
      bg: "EAF2F1",
    },
  ];
  ...
  slide.addText(
    [
      { text: "My next step:  ", options: { bold: true, color: c.accent } },
      { text: "To move up one level, I will ___.", options: { color: T.body } },
    ],
    { x: 0.7, y: 4.35, w: 8.7, h: 0.75, fontFace: BODY, fontSize: 10.5, valign: "middle" },
  );
}
...
function slideDiscussion(slide, pptx, c) {
  ...
  pillText(
    ...
    2.4,
    0.34,
    `💬 ${esc(label).toUpperCase()} DISCUSSION`,
    c.accent,
    T.white,
    9.5,
  );
  ...
  if (stems.length) {
    ...
    sy += 0.4;
    slide.addText(
      stems.map((s) => ({ text: `•  ${esc(s)}`, options: { breakLine: true } })),
      {
        x: 6.85,
        y: sy,
        w: 2.6,
