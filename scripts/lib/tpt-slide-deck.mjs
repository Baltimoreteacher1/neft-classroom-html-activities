...
function buildDragSortSlide(problem, themeEmoji, themeName) {
  ...
  const catLabels = categories
    .map(
      (c) =>
        `<span class="slide-badge" style="background:var(--teal-light); color:var(--navy); margin-right:6px;">${esc(c.label || c)}</span>`,
    )
    .join("");
...
export function buildTptSlideDeck(ctx) {
  ...
  add(
    "👁️ I Do",
    "I Do — Watch Me",
    `${slideHeader(themeEmoji, themeName, esc(iDoTitle))}<div class="slide-card">${renderLines(iDoLines)}</div>`,
  );
  add(
    "🤝 We Do",
    "We Do — Together",
    `${slideHeader(themeEmoji, themeName, esc(weDoTitle))}<div class="slide-card">${renderLines(weDoLines)}</div>`,
  );
  add(
    "✋ You Do",
    "You Do — Your Turn",
  ...
  for (let v = 0; v < Math.min(vocabList.length, 8); v += 2) {
    ...
    add(
      "📝 Vocab",
      `Vocabulary ${v + 1}–${Math.min(v + 2, vocabList.length)}`,
      `${slideHeader(themeEmoji, themeName, "Visual Vocabulary")}<div class="vocab-grid" style="grid-template-columns:1fr 1fr;">${cards}</div>`,
    );
  }
  ...
  add(
    "⚠️ Error",
    "Error Analysis",
    `${slideHeader(themeEmoji, themeName, "Practice Challenge")}${practiceHtml}`,
  );
  ...
  add(
    "📝 Exit",
    "Exit Ticket",
    `${slideHeader(themeEmoji, themeName, "Exit Ticket")}<div class="slide-card">${exitTicketHtml}</div>`,
  );
