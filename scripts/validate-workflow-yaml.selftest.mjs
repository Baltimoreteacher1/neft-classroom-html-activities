...
function check(name, yaml, expectedKeys) {
  const got = findDuplicateKeys(yaml).map((d) => d.key).sort();
...
check("the real codex-verify regression: two top-level `concurrency:`", `
name: Codex Verify
on:
  pull_request:
concurrency:
...
check("duplicate inside one list item", `
steps:
  - name: a
    uses: x
    uses: y
...
check("block scalar contents are text, not keys", `
jobs:
  a:
    steps:
      - name: Claude
...
check("folded scalar with indicator (>-) also skipped", `
with:
  claude_args: >-
    --max-turns 25
    foo: bar
...
check("same key in DIFFERENT list items is legal", `
steps:
  - name: one
    run: a
  - name: two
...
check("same key at different nesting levels is legal", `
on:
  push:
    branches: [main]
  pull_request:
...
check("colon in a value is not a key", `
jobs:
  a:
    container: node:22
    run: echo "time: now"
...
check("commented-out duplicate does not count", `
concurrency:
  group: a
# concurrency:
#   group: b
...
check("sibling jobs with identical inner keys are legal", `
jobs:
  validate:
    runs-on: ubuntu-latest
    steps:
