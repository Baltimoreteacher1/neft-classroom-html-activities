...
export function isTypableAnswer(value) {
  const a = clean(value);
  return a.length > 0 && a.length <= SHORT_ANSWER_MAX && a.split(/\s+/).length <= 4 && !/[.;]\s/.test(a);
}
...
function fromFillTable(item, base, where) {
  ...
  if (!rows.every((r) => r.length === columns.length)) fail(`${where}: fill-table row width != headers`);

  const blanks = [];
  for (const c of cells) {
    if (!inRange(c.row, rows.length)) fail(`${where}: fill-table cell row ${c.row} out of range`);
    if (!inRange(c.col, columns.length)) fail(`${where}: fill-table cell col ${c.col} out of range`);
