  ...
  <body>
    ...
    <div class="wrap">
      ...
      <div class="handoff" id="handoff">
        ...
        <!-- Shown only when this page runs inside the lesson shell's Get Ready
             tab: finishing here hands off to the Warm-Up on the SAME screen
             instead of navigating to the lesson URL. -->
        <button class="go" id="toWarmup" type="button" style="display:none; border:0; cursor:pointer; font-size:1rem;">Continue to Warm-Up ⚡ →</button>
      </div>
    ...
    <script>
      ...
      // ---- hand-off to the lesson Warm-Up -------------------------------
      // When this page is embedded in the lesson shell's "Get Ready" tab, a
      // student who finishes the exit ticket should slide straight into the
      // Warm-Up on the same screen — not be told to "Start Lesson" (a link
      // that, inside the iframe, would load a whole second lesson shell).
      var EXIT_KEYS = ${JSON.stringify(d.exit.map((_, i) => `x${i + 1}`))};
      var EMBEDDED = window.parent !== window;
      var handedOff = false;

      function handOffToWarmup() {
        if (handedOff) return;
        handedOff = true;
        try {
          window.parent.postMessage(
            { type: "nt-readiness-complete", lessonId: ${JSON.stringify(id)} },
            window.location.origin,
          );
        } catch (e) {}
      }

      function checkExitComplete() {
        if (!EMBEDDED || handedOff) return;
        if (!EXIT_KEYS.every(function (k) { return done[k]; })) return;
        var h = document.getElementById("handoff");
        if (h) h.scrollIntoView({ behavior: "smooth", block: "center" });
        // Give the student a beat to read the ✅ before the screen changes.
        setTimeout(handOffToWarmup, 1200);
      }

      if (EMBEDDED) {
        var goLink = document.querySelector(".handoff .go[href]");
        if (goLink) goLink.style.display = "none";
        var warmBtn = document.getElementById("toWarmup");
        if (warmBtn) {
          warmBtn.style.display = "inline-block";
          warmBtn.addEventListener("click", handOffToWarmup);
        }
      }
