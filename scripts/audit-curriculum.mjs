...
if (routes?.redirects) {
  for (const rd of routes.redirects) {
    ...
    if (dest && !/^https?:/i.test(dest)) {
      const abs = resolveSitePath(
        dest.endsWith("/") || /\.[a-z0-9]+$/i.test(dest) ? dest : `${dest}/`,
        join(root, "x.html"),
      );
      if (abs && !targetExists(abs)) {
        add(
          "error",
          "redirect",
          `redirect ${rd.source} → ${dest} points at a missing target`,
          "data/routes.json",
        );
      }
    }
    if (rd.source && routePaths.has(rd.source.replace(/\/$/, "/"))) {
      add(
        "warn",
        "redirect",
        `redirect source ${rd.source} collides with a live route`,
        "data/routes.json",
      );
    }
  }
...
for (const file of htmlFiles) {
  ...
  while ((m = LINK_RE.exec(markup)) !== null) {
    ...
    if (link.includes("${") || /[\n\r]/.test(link) || link.includes("' +") || link.includes('" +'))
      continue;
    ...
    if (!targetExists(abs)) {
      ...
      link.startsWith("/") &&
      !existsExactCase(abs.endsWith("index.html") && link.endsWith("/") ? dirname(abs) : abs)
    ) {
...
const referencedPaths = new Set([...routePaths].map((p) => p.replace(/\/?$/, "/")));
    ...
    while ((m = re.exec(html)) !== null) {
      ...
      if (!existsSync(target)) {
        add(
          "error",
          "resource",
          `${name}: referenced download missing → ${m[1]}`,
          `lessons/${name}/${htmlName}`,
        );
      }
    }
  }
  ...
  if (existsSync(configPath)) {
    ...
    try {
      ...
      add(
        "error",
        "config",
        `${name}: config.json invalid: ${err.message}`,
        `lessons/${name}/config.json`,
      );
      cfg = null;
    }
    if (cfg) {
      ...
      if (cfg.unit != null && Number(cfg.unit) !== prefixUnit && !Number.isNaN(prefixUnit)) {
        add(
          "warn",
          "mapping",
          `${name}: config.unit=${cfg.unit} but lessonId prefix is unit ${prefixUnit}`,
          `lessons/${name}/config.json`,
        );
      }
      if (
        cfg.lessonId &&
        cfg.lessonId !== name &&
        cfg.lessonId !== name.replace(/-flagship$/, "")
      ) {
        add(
          "warn",
          "mapping",
          `${name}: config.lessonId="${cfg.lessonId}" does not match dir name`,
          `lessons/${name}/config.json`,
        );
      }
      // catalog cross-check
      ...
      if (cat) {
        if (cat.unit != null && cfg.unit != null && Number(cat.unit) !== Number(cfg.unit)) {
          add(
            "warn",
            "mapping",
            `${name}: catalog unit ${cat.unit} ≠ config unit ${cfg.unit}`,
            "data/catalog.json",
          );
        }
        if (cat.standard && cfg.standard && cat.standard !== cfg.standard) {
          add(
            "warn",
            "mapping",
            `${name}: catalog standard ${cat.standard} ≠ config standard ${cfg.standard}`,
            "data/catalog.json",
          );
        }
      }
...
// The flagship narrative shell (mission / scenes / finale) was merged INTO the
// base lessons on 2026-08-05, so no `-flagship` twin folders remain. Any that
// reappear alongside their base are a duplicate route and warn here.
for (const name of lessonDirs) {
  ...
  if (baseRef && flagRef) {
    add(
      "warn",
      "flagship",
      `both /lessons/${base}/ and /lessons/${name}/ are surfaced — possible duplicate flagship route`,
      "data/*.json",
    );
  }
}
...
if (FLAGS.write) {
  ...
  writeFileSync(join(dir, "curriculum-audit.json"), JSON.stringify({ summary, findings }, null, 2));
  const md = [
    "# Curriculum Integrity Audit",
    "",
  ...
  for (const [cat, items] of Object.entries(byCategory())) {
    ...
    for (const f of items) {
      md.push(
        `- \`${f.severity.toUpperCase()}\` ${f.message}${f.where ? ` — \`${f.where}\`` : ""}`,
      );
    }
    md.push("");
  }
