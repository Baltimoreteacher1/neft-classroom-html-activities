...
MANIFEST_PATHS.forEach((manifestPath) => {
  ...
  if (Array.isArray(data.lessons)) {
    data.lessons.forEach((lesson) => {
      const qList = Array.isArray(lesson.questions)
        ? lesson.questions
        : lesson.warmup && Array.isArray(lesson.warmup.questions)
          ? lesson.warmup.questions
          : [];
      qList.forEach((q, idx) => {
...
console.log(
  `Shuffled choices and updated correctIndex across ${totalShuffled} questions in do-now lessons-manifest.json!`,
);
