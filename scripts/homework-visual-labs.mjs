...
(function () {
  ...
  function render(topic,v){
    ...
    else if(topic==='factors'){var complete=v.number%v.columns===0,rows=Math.ceil(v.number/v.columns),gap=Math.min(34,320/v.columns,150/rows);body+=dots(v.number,v.columns,110,65,gap)+text(280,235,complete?'Complete rectangle':'Incomplete last row','lab-big','middle');eq=complete?v.columns+' × '+rows+' = '+v.number:v.number+' ÷ '+v.columns+' has a remainder';obs=complete?'The rows are equal, so '+v.columns+' and '+rows+' are a factor pair.':'A factor array needs equal rows with no dots left over.';status=complete?'Factor pair found':'Try another number of columns';mini=Math.min(18,v.number);}
