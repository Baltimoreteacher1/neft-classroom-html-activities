...
| `node tools/import-pacing-baseline.mjs`       | Re-seeds `data/pacing-baseline-2026-27.json` (the ORIGINAL plan) and `data/pacing-unit-ranges.json` from `docs/pacing-sources/plan-baseline.json`. **Run it only to re-seed** — live pacing lives in D1 behind `/api/pacing`, and a re-import does not touch it. `--check` fails if either generated file is stale. | After a source calendar or scope-and-sequence change; never during the school year without re-reading `docs/pacing-sources/planning-notes.md`. |
