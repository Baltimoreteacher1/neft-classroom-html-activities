...
| `npm run generate-math-finder-index`          | Rebuilds `data/math-finder-index.json` (the math-only index powering `/math/finder/`) from `data/registry.json`. Also runs automatically inside `generate-registry`. | After adding/removing a math activity, or editing the folder allowlist in `scripts/generate-math-finder-index.mjs`. |
