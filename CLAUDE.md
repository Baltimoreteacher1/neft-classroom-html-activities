...
| `npm run validate:save-resume`                | Audits Save/Resume wiring across every active activity (0 missing/duplicate/broken required). Part of `validate`.         | Any change near activity state, Save, or Resume — enforced on every `validate` run.         |
