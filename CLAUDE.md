...
| `npm run validate:teacher-credentials`        | **The teacher key must never reach a browser.** One credential set now authorizes both gates: `functions/_lib/teacher-auth.js` holds the accepted keys (Pages env `TEACHER_KEY_NEFT` / `TEACHER_KEY_ALBA`, plus legacy `TEACHER_KEY` for non-browser callers), `/api/teacher-auth/login` exchanges one for a signed HttpOnly cookie, and `_middleware.js` verifies that cookie ONCE per request onto `context.data.teacher` so the page gate and every `/api` write gate agree. Before this, they did not: the planner sent the client-side Teacher Mode PIN as `x-teacher-key`, `/api/pacing` compared it against a different secret, and every planner edit 401'd on a page the teacher had already passed a password gate to reach. This gate cannot grep for a secret's value (a gate holding the credential would be the leak), so it checks the SHAPES that let one escape: a shipped file naming a server-only binding, persisting a teacher key to web storage, sending a raw `x-teacher-key` header from a page, or putting a key in a URL. Four detectors, ten self-tested fixtures. **It is a RATCHET**: nine pre-session teacher surfaces still hold a browser-side key and are listed by name in `MIGRATION_DEBT`; they work today because the cookie authorizes them before the header is read, and the list may only shrink — an unlisted offender fails outright, and a listed file that has been cleaned also fails until its line is deleted. Part of `validate`. | Any change to teacher auth, a teacher-gated `/api` endpoint, or client code that talks to one. |
