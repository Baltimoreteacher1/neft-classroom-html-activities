...
| `npm run validate:pacing-unit-order`         | **The Unit dropdown follows district pacing; lessons inside it follow instructional order.** Two orderings that go wrong by doing nothing. The curriculum numbers its units 1..10 and the district teaches Pre, 3, 4, 6, 7, 8, 9, 5, 2, 10 — so reading the manifest in its own order, or any `.sort()`, produces a dropdown that ASSERTS the wrong sequence, and costs a teacher in November six units of scrolling to reach the one they are in. Lesson ids are strings, so a lexical sort puts 6-10 and 6-11 between 6-1 and 6-2. The order comes from `data/pacing-unit-ranges.json` (generated by `tools/import-pacing-baseline.mjs`; never hand-edit) and this gate holds: the plan is internally sound (contiguous sequence, no unit paced twice, dates non-overlapping); **reconciliation in both directions** — a paced unit the manifest lacks is a dropdown entry with no lessons, and a manifest unit the plan forgets is reported rather than silently appended; the resulting order is neither numeric nor alphabetical, stated as a PROPERTY so it survives a district re-plan; every unit's lessons ascend by numeric suffix; and `assets/curriculum-teacher-planning.js` still reads the plan and does not sort it. A pacing entry with no `curriculumUnit` (MSTAR) is not a Unit option — it owns no lessons and would render an empty Lesson dropdown. It also holds the **ASSEMBLED units** — those whose membership is an instructional decision rather than the curriculum's numbering, authored once in `data/pacing-unit-lessons.json` (ids only, with a written reason; titles resolve from the manifest) — against every surface that renders them: the hub picker, the district crosswalk in `assets/curriculum-district-pacing.js`, and **the dated plan itself**. That last one is the whole point of the check and was missing for the window in which it mattered: the Pre-Unit was corrected to 1-1, 2-6, 2-7, 6-1, 6-2 in the pickers on 2026-08-16 while `data/pacing-baseline-2026-27.json` went on scheduling 1-1 … 1-6, so the Teach dropdown and the pacing guide — Today, Week, Month, Units, Year, and the printed guide — asserted two different Pre-Units in production and neither could see the other. Fix the SOURCE (`docs/pacing-sources/plan-baseline.json`) and re-run the importer; never hand-edit the generated plan. Thirteen mutation tests run BEFORE the real data, because a detector that has stopped firing reports a perfectly ordered curriculum. Part of `validate`. | Any change to the pacing baseline, `data/pacing-unit-ranges.json`, `data/pacing-unit-lessons.json`, the hub picker, or lesson numbering. |
| `npm run validate:lesson-metadata`           | **A lesson page's metadata must describe the page it is on.** `/lessons/2-6-group2/` served `<title>2.6 Small Group · Group 2</title>` beside `og:title` "1.4 Small Group · Group 2"; 175 of 212 lesson index pages disagreed with themselves, whole-group lessons included. The GENERATOR was never wrong — `tools/inject-enterprise-head.js` rebuilds each block from the page's own `<title>` — it was simply never wired into `npm run build`, so it ran once by hand, the sentinel block froze, and the 2026-08-10 renumber moved the titles out from under it. A frozen block is invisible to every check that reads one surface: the HTML is valid, the routes are 200, the page renders. Only a comparison BETWEEN surfaces of the same page can see it. So the injector now runs in `build` (the actual fix) and this is the alarm for the wiring coming loose again: `og:title` against `<title>`, `og:url` and canonical against the page's own directory, and a description naming a different lesson. It also holds the no-JS/boot-guard block, because the same tool owns it and the same freeze removed it — `isLauncherShell` still required an empty `<div id="app">` the small-group shells had stopped having, so re-running the injector would have stripped the guard from ~127 pages. 14 self-tests first, including a replay of the exact 2-6-group2 defect, and the sweep fails below 1,000 injected pages so an injector that stops running cannot report a clean fleet. Part of `validate`. | Any change to `tools/inject-enterprise-head.js`, to a lesson `<title>`, or to a lesson-page generator. |
| `npm run validate:opening-block`             | **The Pre-Unit is the one place where "which unit is this?" has two correct answers**, and every defect here came from a surface collapsing them into one: the picker keying the Pre-Unit "1" and hiding Unit 1's own lessons, the planner labelling the second week of school "Unit 2", the dated plan teaching the Unit 1 arc because the mapping said unit 1. `tools/opening-block-report.mjs` prints Aug 24 – Sep 8 with the PACING unit and the CANONICAL unit side by side (`npm run report:opening-block` for the table); `--check` asserts the block still teaches the authored sequence, still occupies its 11-day budget, and still contains borrowed days. `tools/opening-block.test.mjs` runs the same assertions in `npm test`. Part of `validate`. | Any change to the Pre-Unit sequence, the pacing baseline, or a surface that reports a day's unit. |
