...
(() => {
  ...
  // The NeeDoh Studio is a real soft-body simulation living in
  // needoh-studio.js (physics, audio, FX, progression). This just emits its
  // root markup — the engine self-hydrates any root that lands in the DOM,
  // so it works in a re-rendered card and in a modal alike.
  function renderNeeDohStudioHtml() {
    if (window.NeeDohStudio) return window.NeeDohStudio.html();
    return '<p class="muted">The NeeDoh Studio is still loading — reopen this in a moment.</p>';
  }
  ...
  const VIEWS = {
    ...
    calming() {
      ...
      const needohHtml = card(
        ...
        "A real squishy toy: grab it to stretch, press to squish. Six shapes, six materials, ranks and combos.",
...
Due May 31"></textarea>
    ...
    sports() {
      const api = sports();
      if (!api) return backHeader("Baseball Season", "home");
      ensureSportsCss();
      return (
        backHeader("Baseball Season", "home") + api.renderView(state.sport, { esc, card })
      );
    },
    rewards() {
      const r = state.rewards;
      ...
      // stay available (as a secondary option) only when there is more than one
      // week, so the total button is never a duplicate.
        ...
        ? `<button class="btn primary block" data-act="reward-payout-all" style="margin-bottom:12px">💵 Pay out everything — ${money(
          ...
          <div class="pay-week">
            <div class="pay-week-head">
              <div><b>${
                w.weekKey === thisWk ? "This week so far" : `Week of ${weekLabel(w.weekKey)}`
              }</b><small>${w.daysActive} active day${
                w.daysActive === 1 ? "" : "s"
              }</small></div>
      ...
      return (
        ...
        card(
          ...
          <p class="sub" style="margin:10px 0 0;text-align:center">💡 A grown-up can pay this out any time — no need to wait for Monday.</p>`,
        ) +
        card(
          "pay-rates",
        ...
        .map(
          (w) =>
            `<div class="pay-line"><span>${
              w.weekKey === thisWeekKey() ? "This week so far" : `Week of ${weekLabel(w.weekKey)}`
