...
Due May 31"></textarea>
  ...
  function mergeStates(local, remote) {
    ...
    // 15. Daily check-ins ({ date: {mood, priority} }). No per-entry timestamp,
    // so on a same-date conflict the more recently saved device wins; otherwise
    // union so a check-in made on either device shows on both.
    {
      const ci = { ...(local.checkins || {}) };
      for (const [date, val] of Object.entries(remote.checkins || {})) {
        if (!(date in ci) || remoteIsNewer) ci[date] = val;
      }
      merged.checkins = ci;
    }

    // 16. Today's goal — take the entry for the later date, or the newer device
    // on a same-date tie.
    {
      const l = local.daily || { goal: "", goalDate: "" };
      const r = remote.daily || { goal: "", goalDate: "" };
      if ((r.goalDate || "") > (l.goalDate || "")) merged.daily = r;
      else if ((r.goalDate || "") < (l.goalDate || "")) merged.daily = l;
      else merged.daily = remoteIsNewer ? r : l;
    }

    // 17. captureLog (grow-only per-day marker → union) + garden progress.
    {
      merged.captureLog = { ...(local.captureLog || {}), ...(remote.captureLog || {}) };
      const lg = local.garden || {};
      const rg = remote.garden || {};
      const cfg = remoteIsNewer ? rg : lg;
      merged.garden = {
        // Monotonic progress never goes backward, so max the counters...
        xp: Math.max(lg.xp || 0, rg.xp || 0),
        wateredCount: Math.max(lg.wateredCount || 0, rg.wateredCount || 0),
        // ...and take reservoir spend / plant look from the newer device.
        waterReservoir: Number(cfg.waterReservoir) || 0,
        plantStage: Number(cfg.plantStage) || 0,
        plantType: cfg.plantType || "cactus",
      };
    }

    // 18. Health movement check-ins: union item flags + __paid per day, so a
    // check on either device shows on both and money is counted at most once.
    {
      const mergedHealthLog = { ...((local.health && local.health.log) || {}) };
      const remoteLog = (remote.health && remote.health.log) || {};
      for (const [date, remDay] of Object.entries(remoteLog)) {
        if (!remDay || typeof remDay !== "object") continue;
        const locDay = mergedHealthLog[date] || {};
        const day = { ...locDay };
        for (const [k, v] of Object.entries(remDay)) {
          if (k === "__paid") continue;
          if (v) day[k] = 1;
        }
        const paid = [
        ...
        if (paid.length) day.__paid = [...new Set(paid)];
        mergedHealthLog[date] = day;
      }
      // Keep local health.items/seeded config; only the day log is shared.
      merged.health = { ...(local.health || { log: {} }), log: mergedHealthLog };
    }
