...
function normalizeDue(courseWork) {
  ...
  return new Date(
    Date.UTC(d.year, (d.month || 1) - 1, d.day || 1, t.hours || 12, t.minutes || 0),
  ).toISOString();
}
...
async function syncOneSession(env, sessionKey) {
  ...
  try {
    snapshot.events = await fetchCalendar(data.access_token);
  } catch (error) {
  ...
  await env.NOAM_SCHOOL_KV.put(sessionKey, JSON.stringify(data), {
    expirationTtl: 60 * 60 * 24 * 30,
  });
  await env.NOAM_SCHOOL_KV.put(`cron:${sessionKey}`, JSON.stringify(snapshot), {
    expirationTtl: 60 * 60 * 24 * 30,
  });
...
export default {
    ...
    ctx.waitUntil(
      (async () => {
        ...
        for (const key of list.keys) {
          try {
            results.push(await syncOneSession(env, key.name));
          } catch (error) {
            results.push({ sessionKey: key.name, ok: false, error: error.message });
          }
        }
        await env.NOAM_SCHOOL_KV.put(
          "cron:last-run",
  ...
  async fetch(request, env) {
    ...
    return json({
      ok: true,
      message: "Noam School cron worker is installed. Use /health to inspect last run.",
    });
  },
};
